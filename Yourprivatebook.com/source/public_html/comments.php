...
echo '<br>'.info_panel($info,'<br><a href="#a_comments_show" onclick="javascript:parse_ajax_request(document.getElementById(\'comments_show_form'.$n.'\'),\''.$s[site_url].'/comments.php?&n='.$n.'\',\'comments_show_box'.$n.'\'); check_show_hide_div(\'comments_show_box'.$n.'\'); show_hide_div_id(0,\'enter_comment_box'.$n.'\');">'.$m[show_comments].'</a>');
exit;
}
...
$in = $x[1];
if ($x[0])
{ $a = enter_comment_box($in[n],info_panel($m[errorsfound],implode('<br />',$x[0])));
...
$a = info_panel($m[comment_entered],'<br><a href="#a_comments_show" onclick="javascript:parse_ajax_request(document.getElementById(\'comments_show_form'.$in[n].'\'),\''.$s[site_url].'/comments.php?n='.$in[n].'\',\'comments_show_box'.$in[n].'\'); check_show_hide_div(\'comments_show_box'.$in[n].'\'); show_hide_div_id(0,\'enter_comment_box'.$in[n].'\');">'.$m[show_comments].'</a>');
//if ($s[charset]!='UTF-8') $a = iconv($s[charset],'UTF-8',$a);
echo "<br>$a";
exit;
}
