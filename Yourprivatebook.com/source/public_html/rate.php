...
if ((!$s[GC_u_n]) AND ($s[rate_reg_only]))
{ if ($in[remote]) problem($m[no_logged]);
  else { $s[problem] = $m[no_logged]; rating_table($in[what],$in[n]); } 
}
$s[rated_now] = 1;
if ($in[what]=='u') { $item_vars = get_user_variables($in[n]); $table = "$s[pr]users"; }
else { $item_vars = get_ad_variables($in[n]); $table = "$s[pr]ads"; }
...
$rows = mysqli_num_rows($q); $vyhodit = floor($rows*($s[rate_exclude]/100));
while ($x = mysqli_fetch_assoc($q)) $rates[] = $x[rating]; $votes = count($rates);
