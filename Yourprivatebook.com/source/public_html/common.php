...
if (count(preg_grep("/\.\.\//i",$_REQUEST))) exit;
$_GET = preg_replace("/'\|\[\]/i",'',$_GET);
$_COOKIE = preg_replace("/'|\|/i",'',$_COOKIE);
$_SESSION = preg_replace("/'|\|/i",'',$_SESSION);

/*
if (!$_SESSION[log_country]) $area_redirect = 1;
log_country();
if ($_SESSION[log_country])
{ $q = dq("select flag,name from $s[pr]countries where code = '$_SESSION[log_country]'",1);
  $x = mysqli_fetch_assoc($q);
  if ($x[name]) { $s[this_country] = $x[name]; $s[this_flag] = "$s[site_url]/images/flags/large/$x[flag]"; }
}

//$s[this_country] = 'Sweden';
if (($_SESSION[log_country]) AND ($area_redirect))
{ $q = dq("select n,rewrite_url from $s[pr]areas where level = 1 and title = '$s[this_country]'",1);
  $x = mysqli_fetch_assoc($q);
  if ($x[n]) { header("Location: $s[site_url]/$s[ARfold_l_cat]-0-$x[n]/$x[rewrite_url].html"); exit; }
}
*/
...
if ($s[this_area])
{ $q = dq("select bigboss,rewrite_url,title,level,latitude,longitude from $s[pr]areas where n = '$s[this_area]'",1); $x = mysqli_fetch_assoc($q);
...
if ($user_vars[nick]) $s[user_nick] = $user_vars[nick];

##################################################################################
##################################################################################
##################################################################################

function info_panel($line1,$line2,$what) {
global $s;
if (!$what) $what = 'warning';
$a = '<div class="panel panel-'.$what.'">
<div class="panel-heading">'.$line1.'</div>';
if ($line2) $a .= '<div class="panel-body">'.$line2.'</div>';
$a .= '</div>';
return $a;
}
...
$secure = strtolower(getenv('HTTPS'));
if (($secure=='on') AND (!strstr($s[site_url],'https:'))) $new_http = 'http';
elseif (($secure!='on') AND (strstr($s[site_url],'https:'))) $new_http = 'https';
if ((!$new_host) AND (!$new_http)) return false;
if (!$new_host) $new_host = $http_host;
if (!$new_http) if ($secure=='on') $new_http = 'https'; else $new_http = 'http';
$request = getenv('REQUEST_URI');
if (!$request) $request = '/';
header("Location: $new_http://$new_host$request");
exit;
}
...
if (!mysqli_num_rows($q)) $q = dq("select * from $s[pr]ads_prices where c = '0' order by days",1);
while ($b=mysqli_fetch_assoc($q)) { $a[lines] .= parse_part('ad_prices_one_line.txt',$b); $s[period_options] .= '<option value="'.$b[days].'">'.$b[days].'</option>'; }
...
if (preg_match('/^([a-z0-9]+([_\.\-]{1}[a-z0-9]+)*){1}([@]){1}([a-z0-9]+([_\-]{1}[a-z0-9]+)*)+(([\.]{1}[a-z]{2,10}){1,3}){1}$/i',$email)) return 1;
return 0;
}
...
$q = dq("select count(*) from $s[pr]online",0); $x = mysqli_fetch_row($q);
return $x[0];
}
...
while ($b=mysqli_fetch_assoc($q)) $a .= "<input type=\"radio\" name=\"c1\" value=\"$b[n]\">$b[title]<br>";
return $a;
}
...
$q = dq("select * from $s[pr]cats where alias_of = 0 $where and visible = 1 order by level desc,path_text",1);
...
{ if (!$usit) { $a[hide_usit_begin] = '<!--'; $a[hide_usit_end] = '-->'; }
  ...
  if ($a[x_highlight_by]>$s[cas]) $a[item_style] = 'border:5px solid #FAE672;';
  if ($a[x_bold_by]>$s[cas]) $a[title_style] = 'font-weight:bold;font-size:110%;';
...
global $s,$m;
unset($s[ad_slideshow]);
if ($s[GC_style]!='Mobile') $s[l_columns] = 5;
$width = floor(100/$s[l_columns]);
//list($images,$files,$videos) = get_item_files('a',$numbers,0);
count_impressions($numbers);
foreach ($ads as $k => $a)
...
{ $this_row = '<tr>';
  for ($y=($x-1)*$s[l_columns];$y<=$x*$s[l_columns]-1;$y++)
  $this_row .= $complete_array[$y];
  $this_row .= '</tr>';
  $complete .= $this_row;
  if (!$s[ad_slideshow]) $active = ' active'; else $active = '';
  $s[ad_slideshow] .= '<div class="item'.$active.'" style="padding-left:0px;padding-right:0px;"><table border="0" width="100%" class="table_items_simple">'.$this_row.'</table></div>';
  unset($this_row);
}
...
<div class="item active">
  <img src="#%site_url%#/images/slideshow/1.jpg" alt="">
  <div class="carousel-caption" style="top:20px; z-index: 1;">
    <h3 style="font-size:50px;color:#FFFFFF;text-transform:uppercase;">Get your ads to the world</h3>
    <p><a href="#%site_url%#/advertiser_join.php" style="font-size:20px;color:#FFFFFF;text-transform:uppercase;">Join as an advertiser</a></p>
  </div>
</div>
*/
##################################################################################
...
function check_coupon($price,$coupon,$insert) {
global $s;
$coupon = replace_once_text($coupon);
$q = dq("select * from $s[pr]coupons where code = '$coupon' and minimal_amount <= $price and max_use > used limit 1",1);
$x = mysqli_fetch_assoc($q);
if (!$x[n]) return $price;
if ($x[discount_percent]>0) { $new_price = round($price - ($price * ($x[discount_percent]/100)),2); $s[coupon_percent] = $x[discount_percent]; }
else { $new_price = $price - $x[discount_money]; $s[coupon_money] = $x[discount_money]; }
$s[coupon_discount] = $price - $new_price;
$s[coupon_code] = $x[code];
if ($insert) dq("update $s[pr]coupons set used = used + 1 where n = '$x[n]'",1);
return $new_price;
}
...
while ($x = mysqli_fetch_assoc($q)) $a[$x[n]] = $x;
return $a;
...
$vl[year] = $s[year];
if (!$vl[nick]) $vl[nick] = $s[user_nick];
...
/*
for ($x=1;$x<=10;$x++) if ($s["banner_left$x"]) $banner_left[] = $s["banner_left$x"];
shuffle($banner_left);
for ($x=0;$x<=2;$x++) $vl[banner_left] .= '<p>'.$banner_left[$x].'</p>';
for ($x=1;$x<=10;$x++) if ($s["banner_right$x"]) $banner_right[] = $s["banner_right$x"];
shuffle($banner_right);
for ($x=0;$x<=2;$x++) $vl[banner_right] .= '<p>'.$banner_right[$x].'</p>';
for ($x=1;$x<=10;$x++) if ($s["banner_bottom$x"]) $banner_bottom[] = $s["banner_bottom$x"];
shuffle($banner_bottom);
for ($x=0;$x<=2;$x++) $vl[banner_bottom] .= '<div style="display:inline;">'.$banner_bottom[$x].'</div>';
*/
...
if (($s[show_qr]) AND ($qrimage=show_qrcode($vl[this_url]))) $vl[qrimage] = '<div style="text-align:center;"><img border="0" src="'.$qrimage.'"></div>';
//echo $vl[qrimage];
...
$q = dq("select count(*) from $s[pr]visits_today where ip = '$s[ip]' and old = 0",1); $pocet = mysqli_fetch_row($q);
if (!$pocet[0]) dq("insert into $s[pr]visits_today values ('$s[ip]',$s[cas],0)",1);
$q = dq("select count(*) from $s[pr]visits_today",1); $pocet = mysqli_fetch_row($q); $vl[todays_visits] = $pocet[0];
*/

if ($t=='index.html') unset($vl[categories]); else $vl[categories] = get_categories_list();
if ($vl[categories]) 
...
if ($t=='index.html') unset($vl[areas]); else $vl[areas] = get_areas_list();
if ($vl[areas]) 
...
{ $vl[hide_area_info_begin] = '<!--';  $vl[hide_area_info_end] = '-->';
  if (!$vl[hide_areas_begin]) $vl[hide_all_areas] = 'style="display:none;"';
}
...
{ unset($vl[offer_wanted]);
  if ($s[this_offer_wanted]=='offer') { $s[ads_type] = $m[offer]; $vl[offer_wanted] .= "<p><b>$m[offer]</b></p>"; } else { $vl[offer_wanted] .= '<p><a rel="nofollow" class="link10" href="'.str_replace('extra_commands','offer',str_replace($s[site_url].'/extra_commands',$s[site_url].'/index_offer',$s[offer_wanted_base])).'">'.$m[offer].'</a></p>'; }
  if ($s[this_offer_wanted]=='wanted') { $s[ads_type] = $m[wanted]; $vl[offer_wanted] .= "<p><b>$m[wanted]</b></p>"; } else { $vl[offer_wanted] .= '<p><a rel="nofollow" class="link10" href="'.str_replace('extra_commands','wanted',str_replace($s[site_url].'/extra_commands',$s[site_url].'/index_wanted',$s[offer_wanted_base])).'">'.$m[wanted].'</a></p>'; }
  if ($s[this_offer_wanted]=='all') { $s[ads_type] = $m[offer_wanted]; $vl[offer_wanted] .= "<p><b>$m[offer_wanted]</b></p>"; } else { $vl[offer_wanted] .= '<p><a rel="nofollow" class="link10" href="'.str_replace('extra_commands','all',str_replace($s[site_url].'/extra_commands',$s[site_url].'/index_all',$s[offer_wanted_base])).'">'.$m[offer_wanted].'</a></p>'; }
}
...
while ($x = mysqli_fetch_assoc($q)) $vl[$x[mark]] = $x[html];
include("$s[phppath]/data/info.php");
...
else { $vl[hide_for_no_user_begin] = '<!--'; $vl[hide_for_no_user_end] = '-->'; $vl[hide_for_no_user_begin1] = '<!--'; $vl[hide_for_no_user_end1] = '-->'; }
...
{ $style_n++;
  ...
  if ($v==$s[GC_style]) $vl[menu_styles] .= "<li><a href=\"$s[site_url]/styles.php?style=$v\" style=\"TEXT-TRANSFORM: uppercase;\"> :".str_replace('_',' ',$v).": </a></li>";
...
$template = template_select($t,0,$s[GC_style]);
$meta = implode('',file($template));
preg_match('/(\[metatags\])(.*)\[\/metatags\]/isU',$meta,$x);
if (trim($x[2]))
{ preg_match('/(\[title\])(.*)\[\/title\]/isU',$x[2],$x1); if (trim($x1[2])) $vl[meta_title] = $x1[2];
  preg_match('/(\[description\])(.*)\[\/description\]/isU',$x[2],$x1); if (trim($x1[2])) $vl[meta_description] = $x1[2];
  preg_match('/(\[keywords\])(.*)\[\/keywords\]/isU',$x[2],$x1); if (trim($x1[2])) $vl[meta_keywords] = $x1[2];
  preg_match('/(\[footer\])(.*)\[\/footer\]/isU',$x[2],$x1); if (trim($x1[2])) $vl[meta_footer] = $x1[2];
}
/*
[metatags]
[title]meta title here[/title]
[description]meta description here[/description]
[keywords]meta keywords here[/keywords]
[/metatags]
*/
$line = parse_variables_in_template(template_select('_header.txt',0,$s[GC_style]),$vl);
$line .= parse_variables_in_template($template,$vl);
$line .= str_replace('</body',$info.'</body',parse_variables_in_template(template_select('_footer.txt',0,$s[GC_style]),$vl));
...
global $s,$m;
if (!is_array($vl)) $vl = array();
$vl[charset] = $s[charset]; $vl[site_url] = $s[site_url]; $vl[currency] = $s[currency]; $vl[css_style] = $s[GC_style];
$vl[selected_menu] = 0;
...
while ($x=mysqli_fetch_assoc($q)) { $a[$x[n]] = $x; $numbers[] = $x[n]; }
...
{ unset($a);
  $q = dq("select * from $s[pr]cats where parent = '$this_category[parent]' and visible = 1 order by rank,title",1);
  while ($x=mysqli_fetch_assoc($q)) { $a[] = $x; $numbers[] = $x[n]; }
...
$q = dq("select * from $s[pr]cats where $query and visible = 1 order by level",1);
while ($x=mysqli_fetch_assoc($q)) { $a[] = $x; $numbers[] = $x[n]; }
if ($s[show_left_items]) $items = get_item_numbers_cats($s[this_area],$numbers,$s[this_offer_wanted]);
foreach ($a as $k=>$category)
{ if ($s[show_left_items]) $left_items = ' ('.$items[$category[n]][items].')';
...
{ if ($s[show_left_items]) $left_items = ' ('.$items[$area[n]][items].')';
  ...
  if ($area[n]==$s[this_area]) $areas .= '<p><img alt="" src="'.$folder_icon.'">&nbsp;'.$pomlcek.'<a class="link10" href="'.$this_url.'"><b> '.$area[title].$left_items.'</b></a></p>';
  else $areas .= '<p><img alt="" src="'.$folder_icon.'">&nbsp;'.$pomlcek.'<a class="link10" href="'.$this_url.'">'.$area[title].' ('.$area[items].')</a></p>';
}
return $areas;
}
...
get_country();
$_SESSION[log_country] = $s[country];
}
...
{ $q = dq("select * from $s[pr]usit_list_short where c = '$bigboss'",1);
  $usit_list_short = mysqli_fetch_assoc($q);
}

$q = dq("select * from $s[pr]usit_list where category = '$cat' order by rank",1);
while ($x=mysqli_fetch_assoc($q)) $usit_ranks[$x[rank]] = $x[usit_n];
...
if (!$s[share_usit]) { if ($only_bigboss_n) $where .= " AND bigboss = '$only_bigboss_n'"; }
...
{ set_time_limit(30);
  ...
  $a[path_text] = preg_replace("/<%.+%>/i",'',$a[path_text]);
  $a[path_text] = preg_replace("/<%.+$/i",$a[title],$a[path_text]);
...
$payment_process = mysqli_fetch_assoc($q);
...
$q = dq("select * from $s[pr]orders where user = '$s[GC_u_n]' AND n = '$payment_process[order_n]'",1);
$order = mysqli_fetch_assoc($q);
if ($order[info]) $s[info] = info_panel($order[info]);
user_home_page();
}

##################################################################################

function user_login_form_ajax() {
global $s,$m;
$a = replace_array_text($_GET);
$in[email] = $a[email]; $in[password] = $a[password];
if ($s[user_login_captcha]) $in[display_captcha_tr] = 'table-row'; else $in[display_captcha_tr] = 'none';
$in[info] = $s[info];
return stripslashes(parse_part('user_login_form.txt',$in));
}
...
$a = replace_array_text($_POST);
$in[email] = $a[email]; $in[password] = $a[password];
if ($s[user_login_captcha]) $in[field_captcha_test] = parse_part('form_captcha_test.txt',$a);
$in[info] = $s[info];
...
if (($s[post_ads_who]!=3) OR ($user_vars[paid_until]>$s[cas])) { $user_vars[hide_upgrade_begin] = '<!--'; $user_vars[hide_upgrade_end] = '-->'; }
...
global $s;
if ($what=='u') $folder = 'users'; else $folder = 'images';
...
{ $filename = str_replace("$s[site_url]/uploads/$folder/",'',$v[url]);
  ...
  $path_big = str_replace($s[site_url],$s[phppath],"$s[site_url]/uploads/$folder/$big_filename"); $exists_big = file_exists($path_big);
   //echo "((!$exists_small) OR (!$exists_big))";
  if ((!$exists_small) OR (!$exists_big))
  { if ($exists_small) $big_url = copy($path_small,$path_big);
    elseif ($exists_big)
    { $w_small = $s[preview_w]; if (!$w_small) $w_small = '120';
      $h_small = $s[preview_h]; if (!$h_small) $h_small = '120';
  ...
  $pictures++;
  if (!$picture1)
  { if ($what=='u') dq("update $s[pr]users set picture = '$v[url]' where n = '$n'",1);
    else dq("update $s[pr]ads set picture = '$v[url]' where n = '$n' and status != 'queue'",1);
    //echo "update $s[pr] set picture = '$v[url]' where n = '$n' and status != 'queue'";
    $picture1 = 1;
  }

  $full_size_image = '<br><img border="0" src="'."$s[site_url]/uploads/$folder/$big_filename".'" alt="'.$v[description].'"><br>'.$v[description];
  ...
  if (file_exists("$s[phppath]/uploads/$folder/$big_filename")) $big_url = "$s[site_url]/uploads/$folder/$big_filename";
  else $big_url = $v[url];
...
$n = round($n); if (!$n) user_login_page();
if (!$user) $user = get_user_variables($n);
if ((!$user[n]) OR ($password!=$user[password])) user_login_page();
$s[GC_u_password] = $user[password];
$s[GC_u_name] = $user[name];
$s[GC_u_email] = $user[email];
$s[GC_u_n] = $user[n];
...
while ($x=mysqli_fetch_assoc($q)) $bookmarks[$x[n]] = 1;
return $bookmarks;
}
...
while ($x=mysqli_fetch_assoc($q)) $notes[$x[n]] = nl2br($x[notes]);
return $notes;
}
...
elseif (($in[x_featured_by]>$s[cas]) AND ($s[featured_img])) $icon = $s[featured_img];
...
{ $sort = "sort-$v";
  if ($s[this_sort]==$v) $sort_options[] = "<button type=\"button\" class=\"btn btn-default\" disabled=\"disabled\">$m[$v]</button>";
  elseif ($s[category_use_ajax]) $sort_options[] = "<button type=\"button\" class=\"btn btn-default\" onclick=\"show_waiting('content_div_box');show_waiting('content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category&n=$s[this_cat]&area=$s[this_area]&sort=$v&direction=$s[this_direction]&page=1','content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$v&direction=$s[this_direction]&page=1&total=$total','pages_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$v&direction=$s[this_direction]&page=1&total=$total','pages_div_box1');\">$m[$v]</button>";
  else $sort_options[] = '<button type="button" class="btn btn-default" onclick="location.href=\''.str_replace('extra_commands',$sort,$url).'\'">'.$m[$v].'</button>';
}
$a[sortby_options] = implode(' ',$sort_options);


if ($s[category_use_ajax])
{ $a[link_asc] = "show_waiting('content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=asc&page=1','content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=asc&page=1&total=$total','pages_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=asc&page=1&total=$total','pages_div_box1');";
  $a[link_desc] = "show_waiting('content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=desc&page=1','content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=desc&page=1&total=$total','pages_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=desc&page=1&total=$total','pages_div_box1');";
}
else
{ $a[link_asc] = "location.href='".str_replace('extra_commands','direction-asc',$url)."'";
  $a[link_desc] = "location.href='".str_replace('extra_commands','direction-desc',$url)."'";
}

$numbers = category_pages_list_numbers($url,$total,$page);
if ($numbers) $a[pages_list] = $numbers; else { $a[hide_pages_list_begin] = '<!--'; $a[hide_pages_list_end] = '-->'; }

$a[total] = $total;
return add_this_area(parse_part('pages_list.txt',$a));
}


###################################################################################

function category_pages_list_numbers($base_url,$total,$page) {
global $s,$m;
...
{ for ($x=1;$x<=$pages;$x++)
  { if ($s[category_use_ajax])
    { if ($x==$page) $pages_numbers .= '<button type="button" class="btn btn-default" disabled="disabled">'.$x.'</button>'; 
      elseif ((!$s[pages_max_ads]) OR (($x>=($page-$s[pages_max_ads])) AND ($x<=($page+$s[pages_max_ads])))) $pages_numbers .= "<button type=\"button\" class=\"btn btn-default\" onclick=\"show_waiting('content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=$x','content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=$x&total=$total','pages_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=$x&total=$total','pages_div_box1');\">".$x."</button> ";
      if ($s[pages_max_ads])
      { if ($page>1) $link_first = "<button type=\"button\" class=\"btn btn-default\" onclick=\"show_waiting('content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=1','content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=1&total=$total','pages_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=1&total=$total','pages_div_box1');\"><<</button> ";
        if ($page<$pages) $link_last = "<button type=\"button\" class=\"btn btn-default\" onclick=\"show_waiting('content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=$x','content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=$x&total=$total','pages_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=$x&total=$total','pages_div_box1');\">>></button> ";
      }
      if ($x==($page-1)) $link_down = "<button type=\"button\" class=\"btn btn-default\" onclick=\"show_waiting('content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=".($page-1)."','content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=".($page-1)."&total=$total','pages_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=".($page-1)."&total=$total','pages_div_box1');\"><</button> ";
      elseif ($x==($page+1)) $link_up = "<button type=\"button\" class=\"btn btn-default\" onclick=\"show_waiting('content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$s[this_direction]&page=".($page+1)."','content_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$direction&page=".($page+1)."&total=$total','pages_div_box');javascript:parse_ajax_request('','$s[site_url]/ajax.php?action=category_pages_list&n=$s[this_cat]&area=$s[this_area]&sort=$s[this_sort]&direction=$direction&page=".($page+1)."&total=$total','pages_div_box1');\">></button> ";
    }
    else
    { if ($x==$page) $pages_numbers .= '<button type="button" class="btn btn-default" disabled="disabled">'.$x.'</button>'; 
      elseif ((!$s[pages_max_ads]) OR (($x>=($page-$s[pages_max_ads])) AND ($x<=($page+$s[pages_max_ads]))))
      { if ($x==1) $pages_numbers .= '<button type="button" class="btn btn-default" onclick="location.href=\''.str_replace('-page_n',"",$base_url).'\'">'.$x.'</button>';
        else $pages_numbers .= '<button type="button" class="btn btn-default" onclick="location.href=\''.str_replace('-page_n',"-$x",$base_url).'\'">'.$x.'</button>';
      }
      if ($s[pages_max_ads])
      { if ($page>1) $link_first = '<button type="button" class="btn btn-default" onclick="location.href=\''.str_replace('-page_n','',$base_url).'\'">&laquo;&laquo;&laquo;</button>';
        if ($page<$pages) $link_last = '<button type="button" class="btn btn-default" onclick="location.href=\''.str_replace('-page_n',"-$x",$base_url).'\'">&raquo;&raquo;&raquo;</button>';
      }
      if ($x==($page-1))
      { $url = str_replace('-page_n',"-".($page-1),$base_url);
        $link_down = '<button type="button" class="btn btn-default" onclick="location.href=\''.$url.'\'">&laquo;</button>';
        $s[head_pagination] .= "<link rel=\"prev\" href=\"".str_replace('&amp;','&',$url)."\">\n";
      }
      elseif ($x==($page+1))
      { $url = str_replace('-page_n',"-".($page+1),$base_url);
	    $link_up = '<button type="button" class="btn btn-default" onclick="location.href=\''.$url.'\'">&raquo;</button>';
        $s[head_pagination] .= "<link rel=\"next\" href=\"".str_replace('&amp;','&',$url)."\">\n";
      }
    }
  }
  $pages_list = '<div class="btn-group btn-group-sm">'." $link_first$link_down$pages_numbers$link_up$link_last ".'</div>';
}
...
if (trim($error)) $a[info] = $error;
if ($hide_cancel) { $a[hide_cancel] = '1'; $a[hide_cancel_begin] = '<!--'; $a[hide_cancel_end] = '-->'; }
return parse_part('tell_friend_ad.txt',$a);
}
...
$in = replace_array_text($_POST);
if (is_numeric($n))
{ if ($what=='a') { $a = get_ad_variables($n,0); $a[form_title] = $m[contact_ad_owner]; }
  elseif ($what=='u') { $a = get_user_variables($n); $a[form_title] = $m[contact_user]; }
