...
case 'category_image_deleted'		: category_image_deleted($_GET);
}
switch ($_POST[action]) {
...
<tr><td align=center><select class="form-control" name="n">'.categories_selected('c',0,1,1,1,0).'</select></td></tr>
...
<tr><td align="center">
<table border="0" width="100%" cellspacing="0" cellpadding="2" class="inside_table">
<tr><td align="center">Create <select class="form-control" name="n">';
...
{ $data = get_category_variables($in[n]);
  ...
  $has_subcats = mysqli_fetch_row($q);
  if ($data[visible])
  { $visible = ' checked';
...
for ($x=0;$x<=$s[ads_max_simcats];$x++) $similar .= '<select class="form-control" name="similar['.$x.']"><option value=0>None</option>'.categories_selected('c',$sim[$x],1,1,1,0).'</select><br>';
...
<td align="left" valign="top"><select class="form-control" name="parent"><option value=0>None</option>'.$parent.'</select></td>
</tr>';
for ($x=1;$x<=$n_cats;$x++)
{ if ($x>1) unset($label_category_name);
  echo '<tr>
  <td align="left" valign="top" nowrap>'.$label_category_name.'&nbsp;</td>
  <td align="left" valign="top"><input class="form-control" name="title[]" maxlength=255 value="'.$data[title].'" style="width:550px"></td>
  </tr>';
}
echo '<tr>
<td align="left" valign="top">Group<br><span class="text10">You can group categories on the home page by your needs.</span></td>
<td align="left" valign="top"><select class="form-control" name="cat_group"><option value=0>None</option>'.$cat_group.'</select></td>
</tr>';
if (!$in[n])
{ echo '<tr>
  <td align="left" valign="top">Alias of category</td>
  <td align="left" valign="top"><select class="form-control" name="alias_of"><option value=0>None</option>'.$alias_of.'</select></td>
  </tr>';
}
elseif ($data[alias_of])
{ echo '<tr>
  <td align="left" valign="top">Alias of category</td>
  <td align="left" valign="top"><select class="form-control" name="alias_of">'.$alias_of.'</select></td>
  </tr>';
}
echo '<tr>
<td align="left" valign="top" nowrap>Description</td>
<td align="left" valign="top"><textarea class="form-control" style="width:550px;height:150px;" name="description" rows="10">'.$data[description].'</textarea></td>
</tr>';
if (!$data[alias_of])
{ if (!$in[n]) echo '<tr>
  ...
  <td align="left" valign="top"><input type="file" class="form-control" style="width:550px" maxlength=255 name="image2" value="'.$data[image2].'"></td>
  </tr>';
  if ($data[n])
  { if ($data[image2]) $image = '<div id="image_'.$data[n].'_2"><a name="a_'.$data[n].'_2"></a><img border="0" src="'.$data[image2].'"><br><a href="#a_'.$data[n].'_2" onClick="javascript:parse_ajax_request(document.getElementById(\'\'),\''.$s[site_url].'/administration/categories.php?action=category_image_deleted&n='.$data[n].'&image_n=2\',\'image_'.$data[n].'_2\');">Delete this image</a></div>';
    else $image = 'None';
    echo '<tr>
  ...
  <td align="left" valign="top"><input type="file" class="form-control" style="width:550px" maxlength=255 name="image1" value="'.$data[image1].'"></td>
  </tr>';
  if ($data[n])
  { if ($data[image1]) $image = '<div id="image_'.$data[n].'_1"><a name="a_'.$data[n].'_1"></a><img border="0" src="'.$data[image1].'"><br><a href="#a_'.$data[n].'_1" onClick="javascript:parse_ajax_request(document.getElementById(\'\'),\''.$s[site_url].'/administration/categories.php?action=category_image_deleted&n='.$data[n].'&image_n=1\',\'image_'.$data[n].'_1\');">Delete this image</a></div>'; else $image = 'None';
    echo '<tr>
    <td align="left" valign="top" nowrap>Current image</td>
    <td align="left" valign="top">'.$image.'</td>
    </tr>';
  ...
  <td align="left" valign="top"><textarea class="form-control" style="width:550px;height:150px;" name="m_keyword" rows="10">'.$data[m_keyword].'</textarea></td>
  </tr>
  <tr>
  <td align="left" valign="top" nowrap>Meta description</td>
  <td align="left" valign="top"><textarea class="form-control" style="width:550px;height:150px;" name="m_desc" rows="10">'.$data[m_desc].'</textarea></td>
  </tr>
  <tr>
  <td align="left" valign="top" nowrap>Category page template</td>
  <td align="left" valign="top"><select class="form-control" name="tmpl_cat">'.$tmpl_cat.'</select><br><span class="text10">Template which will be used for the whole category page</span></td>
  </tr>
  <tr>
  <td align="left" valign="top" nowrap>One item template</td>
  <td align="left" valign="top"><select class="form-control" name="tmpl_one">'.$tmpl_one.'</select><br><span class="text10">Template which will be used for each classified ad in this category</span></td>
  </tr>
  <tr>
  <td align="left" valign="top" nowrap>Detail page template</td>
  <td align="left" valign="top"><select class="form-control" name="tmpl_det">'.$tmpl_det.'</select><br><span class="text10">Template which will be used for all detail pages for classified ads from this category</span></td>
  </tr>';
  echo '<tr>
  ...
  echo '<tr>
  <td align="left" valign="top" nowrap>'; if ($s[submit_wizard]) echo 'Show this category in ad submit wizard'; else echo 'Allow submissions'; echo '</td>
  ...
  <td align="left" valign="top"><input class="form-control" name="rank" value="'.$data[rank].'" style="width:100px"><br><span class="text10">Use this field to set rank of categories in the left column and also on the home page. It shows these categories by this rank value. Categories with lower ranks are displayed higher.</span></td>
  </tr>';
  if ($in[n]) echo '<tr>
  <td align="left" valign="top" nowrap>Category URL</td>
  <td align="left" valign="top"><input class="form-control" name="rewrite_url" value="'.$data[rewrite_url].'" style="width:550px"><br><span class="text10">Only English letters, numbers and these characters: - _ /. If you let it blank, the script will generate the URL automatically by the Title field.</span></td>
  </tr>
  ';
}
echo '<tr>
...
{ $q = dq("select items,visible from $s[pr]cats where n = '$in[alias_of]'",1);
  $x = mysqli_fetch_assoc($q); $items = $x[items]; $in[visible] = $x[visible];
}
...
{ category_create_edit_usit($in[n]);
  echo '<br><br><a href="prices.php?action=ads_prices_home&c='.$in[n].'">Edit prices for this category and its subcategories</a>';
}
ift();
}
...
if ($s[share_usit]) echo 'These fields are used for ads in all categories';
else echo 'These fields are valid for this category and all its subcategories';
echo '</td></tr>
...
while ($x = mysqli_fetch_assoc($q)) $usits[$x[usit_n]] = $x;
...
{ $rank = get_usit_rank_options();
  ...
  <td align="center" nowrap><input class="form-control" size="35" name="usit['.$item.'][description]" maxlength="100" value="'.$usits[$item][description].'"></td>
...
return '<select class="form-control" name="'.$name.'">'.$rank.'</select>';
}
...
return '<select class="form-control" name="'.$name.'">'.$a.'</select>';
}
...
global $s;
$in[n] = $in[category];
if ($s[share_usit]) $in[category] = 0;
$q = dq("select * from $s[pr]usit_list where category = '$in[category]'",1);
while ($x = mysqli_fetch_assoc($q)) $old[$x[usit_n]] = $x;
...
{ $current = replace_array_text($in[usit][$usit_n]); if (!$current[description]) continue;
  ...
  if (!$current_old) { $current_old[n] = 'NULL'; if (($current[item_type]=='text') OR ($current[item_type]=='textarea') OR ($current[item_type]=='htmlarea')) $current_old[maxlength] = 255; }
...
global $s;
$q = dq("select * from $s[pr]usit_list where n = '$in[n]'",1); $usit_list = mysqli_fetch_assoc($q);
...
<td align="left"><input class="form-control" name="description" maxlength="255" value="'.$usit_list[description].'" style="width:350px"></td>
</tr>
<tr>
...
<td align="left"><input class="form-control" size="10" name="maxlength" value="'.$usit_list[maxlength].'" style="width:50px"><span class="text10"> Maximum is 255 characters</span></td></tr>
<tr>
...
<td align="left"><input class="form-control" name="def_value" maxlength="255" value="'.$usit_list[def_value_text].'" style="width:350px"></td>
</tr>
<tr>
...
{ $a .= '<tr>
  <td align="left"><input class="form-control" name="ranks['.$x[value_code].']" maxlength="100" value="'.$x[rank].'" style="width:40px"></td>
  <td align="center"><input class="form-control" name="values['.$x[value_code].']" maxlength="255" value="'.$x[description].'" style="width:300px"></td>
  </tr>';
...
{ $rank = $biggest_rank + $x;
  $a .= '<tr>
  <td align="left"><input class="form-control" size="5" name="ranks_new[]" maxlength="100" value="'.$rank.'" style="width:40px"></td>
  <td align="center"><input class="form-control" size="55" name="values_new[]" maxlength="255" value="" style="width:300px"></td>
  </tr>';
}
$a .= '</table>';
return $a;
...
{ $q = dq("select count(*) from $s[pr]usit_list where rank = '$in[rank]'",1);
  $x = mysqli_fetch_row($q); if ($x[0]) user_item_error($in,'Entered rank is already in use');
}
...
function category_image_deleted($in) {
global $s;
$in[n] = round($in[n]);
$in[image_n] = round($in[image_n]);
$category_vars = get_category_variables($in[n]);
unlink(str_replace($s[site_url],$s[phppath],$category_vars['image'.$in[image_n]]));
dq("update $s[pr]cats set image$in[image_n] = '' where n = '$in[n]'",1);
die('<b>Image deleted</b>');
}
...
if ($in[alias_of])
{ $q = dq("select items,visible from $s[pr]cats where n = '$in[alias_of]'",1);
  $x = mysqli_fetch_assoc($q); $items = ",items = '$x[items]'"; $in[visible] = $x[visible];
}
...
{ $current_cat = array_rand($to_check);
  ...
  while ($x = mysqli_fetch_assoc($q)) { $list[$x[n]] = $x[title]; $to_check[] = $x[n]; }
...
while ($x = mysqli_fetch_row($q)) dq("insert into $s[pr]cats_disabled values ('$x[0]')",1);
}
...
{ set_time_limit(300);
  ...
  if ($a[level]==1) $categories .= "<br>$mo <a href=\"categories.php?action=category_edit&n=$a[n]\"><big><big>$a[path_text]</big></big></a> ";
  elseif ($a[level]==2) $categories .= "$mo <a href=\"categories.php?action=category_edit&n=$a[n]\"><big>$a[path_text]</big></a> ";
  else $categories .= "$mo <a href=\"categories.php?action=category_edit&n=$a[n]\">$a[path_text]</a> ";

  
  $categories .= "<font color=\"blue\">#$a[n]</font> ";
...
{ if ($x%2) echo '<tr>';
  ...
  <td align="left"><select class="form-control" name="rank[<?PHP echo $x ?>]">
...
{ $pocet ++; if ($pocet<$data[from]) { $line = fgets($f,10000); continue; }
  ...
  if ($enclosed_by)
  { foreach ($pole as $k=>$v) $pole[$k] = preg_replace("/^$enclosed_by/",'',preg_replace("/$enclosed_by\$/",'',$v)); }
