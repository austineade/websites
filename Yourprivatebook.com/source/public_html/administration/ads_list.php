...
if ($s[share_usit]) list($all_user_items_list,$avail_val) = get_category_usit(0,0,0);

foreach ($all_user_items_list as $k => $v) echo "$k - $v<br>\n";
...
<select class="form-control" name="separator"><option value="|">| (recommended)</option><option value=",">,</option><option value=";">;</option></select><br />
...
<td align="left" colspan="3"><input type="file" maxlength="255" name="file_upload" class="form-control" style="width:450px"></td>
</tr>
...
{ $q = dq("select c_path,a_path from $s[pr]ads where $query and status != 'queue'",1);
  while ($x=mysqli_fetch_assoc($q)) { $a_paths[] = $x[a_path]; $c_paths[] = $x[c_path]; }
  if ($in[to_do]=='enable')
  { dq("update $s[pr]ads set status = 'enabled' where $query",1);
    $info = 'Ads Enabled: '.mysqli_affected_rows($s[db]);
  }
  elseif ($in[to_do]=='disable')
  { dq("update $s[pr]ads set status = 'disabled' where $query",1);
    $info = 'Ads Disabled: '.mysqli_affected_rows($s[db]);
  }
...
$count = mysqli_fetch_row($q); $count = $count[0];
...
{ $rozcesti = '
  ...
  Show classifieds with begin of&nbsp;&nbsp;<select class="form-control" name="from"><option value="1">1</option>';
...
while ($x = mysqli_fetch_assoc($q)) { $ads[$x[n]] = $x; $numbers[] = $x[n]; }
...
/*  if ((!$administrator[allads]) AND ((count($ad[categories])>1) OR (!in_array($ad[categories][0],$s[allowed_cats_l]))) )
  ...
  { $s[no_copy_to_queue] = 1;
    ...
    $owner = get_usern($ad[owner_email]);
    ...
    { dq("delete from $s[pr]ads where n = '$n' and status != 'queue'",1);
      ...
      while ($x=mysqli_fetch_assoc($q)) $new_files[$x[n]] = $x[filename];
      $q = dq("select * from $s[pr]files where item_n = '$n' and what = 'a' and queue = '0'",1);
      while ($x=mysqli_fetch_assoc($q)) $old_files[$x[n]] = $x[filename];
  ...
  { $q = dq("select * from $s[pr]files where item_n = '$n' and what = 'a' and queue = '1'",1);
    while ($x=mysqli_fetch_assoc($q)) $queue_files[$x[n]] = $x[filename];
    $q = dq("select * from $s[pr]files where item_n = '$n' and what = 'a' and queue = '0'",1);
    while ($x=mysqli_fetch_assoc($q)) $old_files[$x[n]] = $x[filename];
...
$ad[url] = get_detail_page_url('ad',$ad[n],$ad[rewrite_url],0);
$ad[n] = $n;
return $ad;
}
...
$count = mysqli_num_rows($q); if (!$count) $count = 0;
...
{ echo '<script language="Javascript">
  ...
  <form method="get" action="ads_list.php" name="myform" id="myform">'.check_field_create('admin').'<input type="hidden" name="action" value="ads_updated_multiple">';
  while ($url = mysqli_fetch_row($q))
  ...
  <select class="form-control" name="to_do"><option value="0">No action</option>
...
{ while ($url = mysqli_fetch_row($q))
  { dq("delete from $s[pr]ads where url = '$url[0]' AND not(n = '$url[1]')",1);
    $affected = $affected + mysqli_affected_rows($s[db]);
  }
...
global $s;
while ($x = mysqli_fetch_assoc($q)) { $ads[$x[n]] = $x; $ad_numbers[] = $x[n]; }
...
<td align="left" valign="top"><input class="form-control" name="n" size=10 maxlength=10></td></tr>
<tr>
<td align="left" valign="top" nowrap>Category</td>
<td align="left" valign="top"><select class="form-control" name="c"><option value="0">Any category</option>'.$categories.'</select></td>
</tr>
<tr>
<td align="left" valign="top" nowrap>Category</td>
<td align="left" valign="top" nowrap><select class="form-control" name="bigboss"><option value="0">Any category</option>'.$categories_first.'</select> incl. subcategories</td>
</tr>
<tr>
<td align="left" valign="top" nowrap>Area</td>
<td align="left" valign="top"><select class="form-control" name="a"><option value="0">Any area</option>'.$areas.'</select></td>
</tr>
<tr>
<td align="left" valign="top" nowrap>Area</td>
<td align="left" valign="top" nowrap><select class="form-control" name="a_bigboss">'.str_replace('All areas','Any area',$areas_first).'</select> incl. subareas</td>
</tr>
<tr>
<td align="left" valign="top" nowrap>Any field contains </td>
<td align="left" valign="top"><input class="form-control" name="any" maxlength="100" style="width:550px"></td>
</tr>
<tr>
<td align="left" valign="top" nowrap>Title contains </td>
<td align="left" valign="top"><input class="form-control" name="title" maxlength="100" style="width:550px"></td>
</tr>
<tr>
<td align="left" valign="top" nowrap>Description contains </td>
<td align="left" valign="top"><input class="form-control" name="description" maxlength="100" style="width:550px"></td>
</tr>
<tr>
<td align="left" valign="top" nowrap>Detailed entry contains </td>
<td align="left" valign="top"><input class="form-control" name="detail" maxlength="100" style="width:550px"></td>
</tr>
<tr>
<td align="left" valign="top" nowrap>URL contains </td>
<td align="left" valign="top"><input class="form-control" name="url" maxlength="100" style="width:550px"></td>
</tr>
<tr>
<td align="left" valign="top" nowrap>Contact name contains </td>
<td align="left" valign="top"><input class="form-control" name="name" maxlength="100" style="width:550px"></td>
</tr>
<tr>
<td align="left" valign="top" nowrap>Contact email contains </td>
<td align="left" valign="top"><input class="form-control" name="email" maxlength="100" style="width:550px"></td>
</tr>
<tr>
<td align="left" valign="top" nowrap>Contact phones contain </td>
<td align="left" valign="top"><input class="form-control" name="pub_phone" maxlength="100" style="width:550px"></td>
</tr>';
...
<td align="left" valign="top"><select class="form-control" name="valid"><option value="0">N/A</option><option value="yes">Classifieds which are currently valid</option><option value="no">Classifieds which are currently not valid</option><option value="future">Classifieds which will valid in the future</option><option value="past">Classifieds which have been valid in the past</option></select></td>
</tr>
<tr>
...
<td align="left" valign="top"><select class="form-control" name="perpage">
...
<select class="form-control" name="sort"><option value="title">Title</option><option value="clicks_total">Popularity (clicks this month)</option><option value="created">Date created</option><option value="name">Contact name</option></select>
...
$count = mysqli_fetch_row($x); $count = $count[0];
...
{ $rozcesti = '<form method="get" action="ads_list.php">'.check_field_create('admin').'
  ...
  $rozcesti .= 'Show classifieds with begin of <select class="form-control" name="from"><option value="1">1</option>';
  $y = ceil($count/$in[perpage]);  
