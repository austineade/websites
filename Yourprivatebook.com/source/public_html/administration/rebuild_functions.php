...
{ $q = dq("select n from $s[pr]ads where $s[where_fixed_part]",1);foreach ($x as $k=>$v) echo "$k - $v<br>";
  while ($x = mysqli_fetch_assoc($q)) { update_item_index($what,$x[n]); increase_print_time(2,1); }
}
...
while ($x = mysqli_fetch_row($q)) $numbers[] = $x[0];
...
while ($x = mysqli_fetch_assoc($q)) dq("update $s[pr]ads set popular = '1' where n = '$x[n]'",1);
...
$cats = mysqli_fetch_assoc($q);
$q = dq("select count(*) as total from $s[pr]areas",0);
$areas = mysqli_fetch_assoc($q);
...
$active = mysqli_fetch_assoc($q);
$q = dq("select count(*) as total from $s[pr]ads where status = 'queue'",0);
$queue = mysqli_fetch_assoc($q); if (!$queue[total]) $queue[total] = 0;
$q = dq("select count(*) as total from $s[pr]ads_abuse_reports",0);
$abuse = mysqli_fetch_assoc($q); if (!$abuse[total]) $abuse[total] = 0;
$q = dq("select count(*) as total from $s[pr]users",0);
$users = mysqli_fetch_assoc($q); if (!$users[total]) $users[total] = 0;
$q = dq("select count(*) as posts from $s[pr]board",0);
$board = mysqli_fetch_assoc($q); if (!$board[posts]) $board[posts] = 0;
$q = dq("select count(*) as records from $s[pr]log_search",0);
$search = mysqli_fetch_assoc($q);
$data = "<?PHP\n
\$s[t_cats] = $cats[total];
...
$q = dq("select * from $s[pr]cats where in_menu = '1' order by rank,title",1);
while ($c = mysqli_fetch_assoc($q))
{ $number++;
  $static[0][0][x][menu_categories] .= "<li><a href=\"".category_url('ad',$c[n],$c[alias_of],1,$c[rewrite_url])."\">$c[title] </a></li>";
}
$q = dq("select * from $s[pr]areas where in_menu = '1' order by title",1);
while ($c = mysqli_fetch_assoc($q))
{ $number++;
  $url = "$s[site_url]/$s[ARfold_l_cat]-0-$c[n]/$c[rewrite_url].html";
  $static[0][0][x][menu_areas] .= "<li><a href=\"$url\">$c[title] </a></li>";
}
...
global $s;
if (!$s[bookmarks_cats_email]) dq("delete from $s[pr]u_to_email",1);
$q = dq("select * from $s[pr]u_to_email",1);
while ($x=mysqli_fetch_assoc($q))
{ $item = get_ad_variables($x[n]);
  if ($item[status]!='enabled') continue;
  $email[title] = $item[title]; $email[url] = get_detail_page_url('ad',$item[n],$item[rewrite_url],1);
  if (!$item[c]) continue;
  $c_array = explode(' ',str_replace('_','',$item[c])); $query = my_implode("$s[pr]u_favorites.n",'or',$c_array);
  $q1 = dq("select $s[pr]users.*,$s[pr]u_favorites.n as category from $s[pr]u_favorites,$s[pr]users where $s[pr]u_favorites.what = 'c' and ($query) and $s[pr]u_favorites.user = $s[pr]users.n",1);
  while ($user = mysqli_fetch_assoc($q1))
  { if (!$categories[$user[category]]) { $c = get_category_variables($user[category]); $categories[$user[category]] = $c[title]; }
    $email[to] = $user[email]; $email[category] = $categories[$user[category]];
    mail_from_template('user_new_ads.txt',$email);
    unset($email);
    set_time_limit(30);
  }
}
dq("truncate table $s[pr]u_to_email",1);
...
{ list($usits,$avail_val) = get_category_usit($cat[n],0,0,1);
  ...
  { if (($usit[item_type]=='text') OR ($usit[item_type]=='textarea') OR ($usit[item_type]=='htmlarea'))
    { $b[content] = $usit[description].'<br>';
      $b[content] .= '<input name="usit['.$usit[usit_n].']" size=20 maxlength=50 class="form-control" value="#_usit_value_'.$usit[usit_n].'_#">';
    ...
    { $b[content] = $usit[description].'<br>';
      $b[content] .= '<select class="form-control" name="usit['.$usit[usit_n].']"><option value="0"#_usit_selected_'.$usit[usit_n].'_0_#>'.$m[any].'</option>';
      foreach ($avail_val[$usit[n]] as $k1=>$one_option)
...
$vl[logo_url] = $s[logo_url]; $vl[css_style] = $s[GC_style];
$vl[banner_code] = $s[banner_code]; $vl[banner_code_left] = $s[banner_code_left]; $vl[banner_code_right] = $s[banner_code_right];
...
//$start = time();
//for ($xx=1;$xx<=100;$xx++)
{ $q = dq("select * from $s[pr]areas",1); while ($x = mysqli_fetch_assoc($q)) $areas[] = $x[n];
...
while ($x = mysqli_fetch_assoc($q)) update_category_area_paths('a',$x[n]);

$q = dq("select * from $s[pr]cats order by level",1);
while ($x = mysqli_fetch_assoc($q)) update_category_area_paths('c',$x[n]);

$q = dq("select n,path_n from $s[pr]cats",1); while ($x = mysqli_fetch_assoc($q)) $cats[$x[n]] = $x[path_n];
$q = dq("select n,path_n from $s[pr]areas",1); while ($x = mysqli_fetch_assoc($q)) $areas[$x[n]] = $x[path_n];
...
if ($s[y_sitemap_cats])
{ $q = dq("select * from $s[pr]cats where visible = '1' AND alias_of = '0' order by path_text",1);
  while ($a=mysqli_fetch_assoc($q)) $list .= sitemap_replace(category_url('ad',$a[n],$a[alias_of],1,$a[rewrite_url]))."\n";
  fwrite($file,$list); unset($list);
}
if ($s[y_sitemap_ads])
{ $q = dq("select * from $s[pr]ads where $s[where_fixed_part] order by n",1);
  while ($a=mysqli_fetch_assoc($q)) $list .= get_detail_page_url('ad',$a[n],$a[rewrite_url],1)."\n";
  fwrite($file,$list); unset($list);
}
