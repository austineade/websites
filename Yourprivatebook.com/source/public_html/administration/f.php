...
global $s;
if (!$selected) $selected = $s[pp_currency];
foreach ($s[pp_currencies] as $k=>$v)
{ if ($k==$selected) $x = ' selected'; else $x = '';
  $a .= '<option value="'.$k.'"'.$x.'>'.$v.'</option>';
}
return '<select class="form-control" name="'.$field_name.'">'.$a.'</select>';
}
...
global $s;
$a = '<div class="info_line"><img border="0" src="'.$s[site_url].'/images/icon_info.png" style="margin-bottom:-5px;">&nbsp;'.$line1;
if ($line2) $a .= '<br>'.$line2;
$a .= '</div><br>';
return $a;
}
...
function db_connect() {
global $s;
if ($s[db]) mysqli_close($s[db]);
$mysqli = mysqli_init();
if (!$mysqli) die('mysqli_init failed');
$mysqli->options(MYSQLI_OPT_CONNECT_TIMEOUT,30);
if (!$mysqli->real_connect($s[dbhost],$s[dbusername],$s[dbpassword],$s[dbname])) {
    die("Unable to connect to the database host. Check database host, username, password.<br>" . mysqli_connect_errno()." - ". mysqli_connect_error());
}
//echo 'Success... ' . $mysqli->host_info . "\n";
mysqli_query($mysqli,"SET SESSION sql_mode = 'MYSQL40'");
$s[db] = $mysqli;
}
...
//if ( ($check) AND (!$q) ) die(mysqli_error($s[db]));
return $q;
}
...
function get_ip() {
global $s;
if ($s[ip_var]=='custom') { if ($s[ip_var_custom]) $s[ip_var] = $s[ip_var_custom]; else unset($s[ip_var]); }
if (!$s[ip_var]) $s[ip_var] = 'REMOTE_ADDR';
if (getenv($s[ip_var])) $ip = getenv($s[ip_var]);
else $ip = $_SERVER[$s[ip_var]];
$x = explode(',',$ip);
$ip = trim($x[0]);
...
list($usec,$sec) = explode(' ',microtime());
$x = $sec+($usec*1000000);
return substr(md5($x.$s[cas]),$from,$to);
}
...
$select[d] = '<select class="form-control" name="'.$select_name.'[d]">'.select_days($date_d).'</select>';
$select[m] = '<select class="form-control" name="'.$select_name.'[m]">'.select_months($date_m).'</select>';
$select[y] = '<select class="form-control" name="'.$select_name.'[y]">'.select_years($date_y).'</select>';
for ($y=1;$y<=3;$y++) if ($s['date_form_'.$y.'a']=='Space') $date_separator[$y] = ' '; elseif ($s['date_form_'.$y.'a']=='Nothing') $date_separator[$y] = ''; else $date_separator[$y] = $s['date_form_'.$y.'a'];
...
require_once("$s[phppath]/phpmailer/class.phpmailer.php");
$mail = new PHPMailer();
if (trim($s[smtp_server]))
{ require_once("$s[phppath]/phpmailer/class.smtp.php");
  $mail->IsSMTP();
