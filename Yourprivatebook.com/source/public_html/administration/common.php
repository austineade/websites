...
if (($categories[0]) AND (!$s[move_main_cat])) $only_bigboss_n = get_bigboss_category($categories[0]);
if (!$in[n]) $in[n] = 0;
for ($x=0;$x<=$s[max_cats]-1;$x++)
{ if (!$x) $b[categories] = 'Categories'; else $b[categories] = '&nbsp;';
  $select_box = '<select class="form-control" name="ad['.$in[n].'][categories][]">';
...
{ if (!$x) $b[areas] = 'Areas'; else $b[areas] = '&nbsp;';
  $select_box = '<select class="form-control" name="ad['.$in[n].'][areas][]">';
  if ($x) $select_box .= '<option value="0">None</option>';
...
{ if (!$v) continue;
  ...
  if ($what=='ad') $a .= '<a target="_self" href="ads_list.php?action=ads_searched&category='.$k.'">'.str_replace('<%','',str_replace('%>','/',$v[path_text])).$info.'</a><br>';
}
return $a;
}
...
{ if (($q != ".") AND ($q != "..") AND (is_file("$s[phppath]/styles/_common/templates/$q")))
  if (preg_match("/^$x[0][a-z0-9_]*\.$x[1]$/",$q)) $pole[] = $q;
}
sort($pole);
foreach ($pole as $k => $v)
...
while ($x = mysqli_fetch_row($q)) $new_list[$x[0]] = $x[1];
...
{ unset($field);
  ...
  { if ($ad_n) $field = $from_database[$usit[n]][value_text]; // edit or queue
    ...
    if ($in=='search_form') $field = '<input maxlength="255" name="user_item['.$usit[n].']" value="'.$field.'" class="form-control" style="width:550px">';
    else $field = '<input maxlength="255" name="'.$fields_name.'[user_item_'.$usit[n].']" value="'.$field.'" class="form-control" style="width:550px">';
  }
  ...
  { if ($ad_n) $field = $from_database[$usit[n]][value_text]; // edit or queue
    ...
    if ($in=='search_form') $field = '<input maxlength="255" name="user_item['.$usit[n].']" value="'.$field.'" class="form-control" style="width:550px">';
    else $field = '<textarea class="form-control" style="width:550px;height:150px;" rows="10" cols="70" name="'.$fields_name.'[user_item_'.$usit[n].']">'.$field.'</textarea>';
  }
  ...
  { if ($ad_n) $field = $from_database[$usit[n]][value_text]; // edit or queue
    ...
    //'<textarea class="form-control" style="width:550px;height:150px;" rows="10" cols="70" name="'.$fields_name.'[user_item_'.$usit[n].']">'.$field.'</textarea>';
  }
  else
  { if ($ad_n) $value = $from_database[$usit[n]][value_code]; // edit or queue
    ...
    { if ($in=='search_form') $field = '<option value="0" selected>Any</option>';
      ...
      if ($in=='search_form') $field = '<select class="form-control" name="user_item['.$usit[n].']">'.$field.'</select>';
      else $field = '<select class="form-control" name="'.$fields_name.'[user_item_'.$usit[n].']">'.$field.'</select>';
    }
    ...
    { $x1 = explode(' ',trim(str_replace('_',' ',$from_database[$usit[n]][value_text])));
      ...
      if ($in=='search_form') $field = '<select class="form-control" style="height:150px;" name="user_item['.$usit[n].']">'.$field.'</select>';
      else $field = '<select class="form-control" style="height:150px;" name="'.$fields_name.'[user_item_'.$usit[n].'][]" size="5" multiple>'.$field.'</select>';
    }
  }
...
if ($parent[n]) { $path_text = str_replace("'",'',"$parent[path_text]%><%$vars[title]"); $path_n = $parent[path_n].$n.'_'; if ($parent[bigboss]) $bigboss = $parent[bigboss]; else $bigboss = $parent[n]; }
...
//foreach ($images[221][1] as $k=>$v) echo "$max -$k - $v<br>";
  ...
  { echo '<tr>
    ...
    <td nowrap align="left" valign="top"><input type="file" maxlength="255" style="width:550px" name="image_upload['.$in[n].']['.$x.']" class="form-control">';    
    ...
    <td nowrap align="left" valign="top"><input maxlength="255" style="width:550px" name="image_description['.$in[n].']['.$x.']" value="'.$images[$in[n]][$x][description].'" class="form-control"></td>
    </tr>';	
  }
}
}
