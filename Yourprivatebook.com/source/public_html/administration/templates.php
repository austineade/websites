...
switch ($_GET[action]) {
case 'templates_home'			: templates_home($_GET[selected_style]);
case 'template_edit'			: template_edit($_GET);
}
switch ($_POST[action]) {
...
<tr><td align="right" valign="top" nowrap height="100%">

<table border="0" cellspacing="0" cellpadding="0" height="100%" class="common_table">
...
echo $s[info];
if ($in[test_email])
{ mail_from_template(str_replace('email_templates','',$in[template]),$data);
  echo info_line('Email sent');
}
...
<tr><td align="left" nowrap><textarea class="form-control" style="width:800px;height:500px;" name="html" rows="40">'.$ct.'</textarea></td></tr>
<tr><td align="center" nowrap>
<input type="submit" name="A1" value="Save" class="button10"><br />
</td></tr></form></table><br /><br />';
if (strstr($in[template],'email_templates')) echo '<a href="templates.php?action=template_edit&selected_style='.$in[selected_style].'&template='.$in[template].'&test_email=1">Send a test email to '.$s[mail].'</a><br /><br />';
echo '<a href="templates.php?action=templates_home&selected_style='. $in[selected_style].'">Back</a><br /><br />';
ift();
}
