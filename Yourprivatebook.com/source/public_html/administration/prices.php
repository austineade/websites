...
case 'users_prices_home'	: users_prices_home();
}
switch ($_POST[action]) {
case 'ads_prices_edited'	: ads_prices_edited($_POST);
case 'users_prices_edited'	: users_prices_edited($_POST);
}
...
function users_prices_edited($in) {
global $s;
dq("delete from $s[pr]u_prices",1);
for ($x=0;$x<=9;$x++)
{ $days = $in[days][$x]; $price = str_replace(',','',$in[price][$x]);
  if (!$price) continue;
  dq("insert into $s[pr]u_prices values ('$in[level]','$days','$price')",1);
}
$s[info] = info_line('Prices Updated');
users_prices_home($in[c]);
}

##################################################################################

function users_prices_home() {
global $s;
$level = 1;
include("$s[phppath]/data/data_forms.php");
$q = dq("select * from $s[pr]u_prices where level = '$level' order by days",1);
while ($x=mysqli_fetch_assoc($q)) $prices[] = $x;
ih();
echo $s[info];
echo '<form action="prices.php" method="post">'.check_field_create('admin').'
<input type="hidden" name="action" value="users_prices_edited">
...
echo '<TR><TD align="center" colspan="11">This feature is used if you selected that only paying users can submit classifieds. The membership fees are editable on this page.</td></tr>';
echo '<tr>
<td align="left" nowrap>Number of days</td>';
for ($x=0;$x<=9;$x++) echo '<td align="center"><input class="form-control" name="days['.$x.']" value="'.$prices[$x][days].'" size=5 maxlength=10></td>';
echo '</tr>';
echo '<tr>
<td align="left">Price </td>';
for ($x=0;$x<=9;$x++) echo '<td align="center"><input class="form-control" name="price['.$x.']" value="'.$prices[$x][price].'" size=5 maxlength=10></td>';
echo '</tr>';
...
if (mysqli_num_rows($q)) $category_specific = 1;
...
for ($x=0;$x<=9;$x++)
echo '<td align="center"><input class="form-control" name="price['.$x.'][days]" value="'.$prices[$x][days].'" size=5 maxlength=10></td>';
echo '</tr>';
...
{ echo '<tr>
  ...
  { if ($k=='ad') $disabled = $prices[$x][disabled];
    echo '<td align="center"><input class="form-control" name="price['.$x.']['.$k.']" value="'.number_format($prices[$x][$k],2).'" size=5 maxlength=10'.$disabled.'></td>';
  }
  echo '</tr>';
}
echo '<tr><td align="center" colspan="11"><input type="submit" name="submit" value="Submit" class="button10"></td></tr>
</form></table></td></tr></table>';
if ($category_specific) echo '<br><a href="prices.php?action=ads_prices_home&c='.$c.'&load=1">Cancel prices for this category and load default prices to the table above.</a><br>';
echo '<br>
<table border="0" width="800" cellspacing="0" cellpadding="0" class="common_table">
