...
case 'area_image_deleted'		: area_image_deleted($_GET);
}
switch ($_POST[action]) {
...
<tr><td align=center><select class="form-control" name="n">'.areas_selected(0,1,1,1,0).'</select></td></tr>
...
{ if ($x>1) unset($label_area_name);
  ...
  <td align="left" valign="top"><input class="form-control" style="width:550px" name="title[]" maxlength=255 value="'.$data[title].'"></td>
  </tr>';
}
echo '<tr>
...
if ($data[n])
{ if ($data[image2]) $image = '<div id="image_'.$data[n].'_2"><a name="a_'.$data[n].'_2"></a><img border="0" src="'.$data[image2].'"><br><a href="#a_'.$data[n].'_2" onClick="javascript:parse_ajax_request(document.getElementById(\'\'),\''.$s[site_url].'/administration/areas.php?action=area_image_deleted&n='.$data[n].'&image_n=2\',\'image_'.$data[n].'_2\');">Delete this image</a></div>';
  else $image = 'Icon not defined. The default icon is used.';
  echo '<tr>
...
echo '<tr>
<td align="left" valign="top" nowrap>Skip to this area when a visitor comes from </td>
<td align="left" valign="top"><select class="form-control" name="country_skip"><option value="">N/A</option>';
$q = dq("select * from $s[pr]countries order by name",1);
while ($x = mysqli_fetch_assoc($q)) 
{ if (($data[country_skip]) AND ($x[code]==$data[country_skip])) $selected = ' selected'; else $selected = '';
  echo '<option value="'.$x[code].'"'.$selected.'>'.$x[name].'</option>';
}
echo '</td>
</tr>';
...
{ echo '<tr>
  ...
  <td align="left" valign="top"><input class="form-control" style="width:550px" name="map_address" maxlength=255 value="'.$data[map_address].'"></td>
  </tr>
  <tr>
  <td align="left" valign="top" nowrap>Map co-ordinates </td>
  <td align="left" valign="top">Latitude: <input class="form-control" style="width:150px" name="latitude" maxlength=255 value="'.$data[latitude].'"> Longitude: <input class="form-control" style="width:150px" name="longitude" maxlength=255 value="'.$data[longitude].'"><br><span class="text10">Keep these fields empty to automatically count these values for the address entered above<br></span></td>
  </tr>';
}
if ($data[level]==1)
{ echo '<tr>
  <td align="left" valign="top" nowrap>Map zoom </td>
  <td align="left" valign="top"><select class="form-control" name="map_zoom">';
  unset($selected);
...
echo '<tr>
<td align="left" valign="top" nowrap>'; if ($s[submit_wizard]) echo 'Show this area in ad submit wizard'; else echo 'Allow submissions'; echo '</td>
...
<td align="left" valign="top"><input class="form-control" name="rank" value="'.$data[rank].'" style="width:100px"><br><span class="text10">Use this field to set rank of areas in the left column. It shows these areas by this rank value. Areas with lower ranks are displayed higher.</span></td>
</tr>
';
if ($in[n]) echo '<tr>
...
{ $v = replace_once_text($v);
  ...
  dq("insert into $s[pr]areas values (NULL,'$v','$in[parent]','0','$level','$in[rank]','','','','','$in[submit_here]','$similar','$in[in_menu]','$in[country]','','0','0','0','$in[country_skip]','$in[map_address]','','','7','','','','')",1);
...
$in[description] = refund_html($in[description]);
foreach ($in as $k=>$v) $in[$k] = str_replace('&lt;','<',str_replace('&gt;','>',$v));
dq("update $s[pr]areas set parent = '$in[parent]', rank = '$in[rank]', title = '$in[title]', country_skip = '$in[country_skip]', map_address = '$in[map_address]', map_zoom = '$in[map_zoom]', submit_here = '$in[submit_here]', in_menu = '$in[in_menu]', similar = '$similar', rewrite_url = '$in[rewrite_url]' where n = '$in[n]'",1);
...
{ set_time_limit(300);
  ...
  if ($a[level]==1) $areas .= "<br>$mo <a href=\"areas.php?action=area_edit&n=$a[n]\"><big><big>$a[path_text]</big></big></a> ";
  elseif ($a[level]==2) $areas .= "$mo <a href=\"areas.php?action=area_edit&n=$a[n]\"><big>$a[path_text]</big></a> ";
  else $areas .= "$mo <a href=\"areas.php?action=area_edit&n=$a[n]\">$a[path_text]</a> ";
  
  $areas .= "<font color=\"blue\">#$a[n]</font> ";
  ...
  if ($a[level]==1) $areas .= " - <a class=\"link10\" href=\"ads_list.php?action=ads_searched&a_bigboss=$a[n]\">Classified ads incl. subareas</a>";
...
while ($a=mysqli_fetch_assoc($q)) $list[] = stripslashes($a[name]);
...
{ $query = my_implode('code','or',$in[import_country]);
  ...
  while ($country = mysqli_fetch_assoc($q)) import_country($country);
}
if (is_array($in[import_areas]))
{ $query = my_implode('code','or',$in[import_areas]);
  $q = dq("select * from $s[pr]countries where $query",1); while ($country = mysqli_fetch_assoc($q)) $countries[$country[code]] = $country;
  $query = my_implode('country','or',$in[import_areas]);
  $q = dq("select * from $s[pr]areas where $query",1); while ($area = mysqli_fetch_assoc($q)) $areas[$area[country]] = $area;
  $query = my_implode('country','or',$in[import_areas]);
  $q = dq("select * from $s[pr]countries_regions where $query",1);
  while ($region = mysqli_fetch_assoc($q))
...
global $s;
$q = dq("select * from $s[pr]areas where country = '$country[code]'",1); $a = mysqli_fetch_assoc($q); if ($a[n]) return $a[n];
...
function area_image_deleted($in) {
global $s;
$in[n] = round($in[n]);
$in[image_n] = round($in[image_n]);
$area_vars = get_area_variables($in[n]);
unlink(str_replace($s[site_url],$s[phppath],$area_vars['image'.$in[image_n]]));
dq("update $s[pr]areas set image$in[image_n] = '' where n = '$in[n]'",1);
