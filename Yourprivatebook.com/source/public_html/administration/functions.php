...
if (file_exists('./data/data.php')) include('./data/data.php'); else include('../data/data.php');
include_once("$s[phppath]/administration/f.php");
ini_set("magic_quotes_sybase",0);
ini_set("magic_quotes_gpc",0);
ini_set("magic_quotes_runtime",0);
session_start();
foreach ($s as $k=>$v) if (!is_object($v)) $s[$k] = str_replace("&#039;","'",$v);
$s[rewr_delim] = '/';
...
//echo get_ipnum('1.0.64.0');exit;
function get_ipnum($in) {
global $s;
if ($in) $x = explode('.',$in);
elseif ($s[have_ip]) $x = explode('.',$s[have_ip]);
else $x = explode('.',trim($s[ip]));
//$x = explode('.','61.88.255.255'); // AU
//$x = explode('.','85.207.23.172'); // Vrac
...
global $s;
/*
$record = geoip_record_by_name($s[have_ip]);
if ($s[country]=$record[country_code])
{ if ((!$s[ad_region_target]) AND (!$s[ad_city_target])) return $s[country];
  if (($s[region]=$record[region]) AND ($s[ad_region_target]))
  { if (($s[city_name]=$record[city]) AND ($s[ad_city_target]))
    { $q = dq("select n from $s[pr]cities where country = '$s[country]' and region = '$s[region]' and name = '$s[city_name]' limit 1",1);
      $x = mysqli_fetch_assoc($q);
      $s[city] = $x[n];
      return true;
    }
    elseif (!$s[ad_city_target])
    { return true;
    }
  }
}
*/
if ($s[country]=getenv('GEOIP_COUNTRY_CODE'))
{ if ((!$s[ad_region_target]) AND (!$s[ad_city_target])) return $s[country];
  if (($s[region]=getenv('GEOIP_REGION')) AND ($s[ad_region_target]))
  { if (($s[city_name]=getenv('GEOIP_CITY')) AND ($s[ad_city_target]))
    { $q = dq("select city from $s[pr]cities where country = '$s[country]' and region = '$s[region]' and city_name = '$s[city_name]' limit 1",1);
      $x = mysqli_fetch_assoc($q);
      $s[city] = $x[city];
      return true;
    }
...
if (!$s[ipnum]) get_ipnum();
if (!$s[ipnum]) return '';
$q = dq("select country,region,city,city_name from $s[pr]ip_country_temp where n = '$s[ipnum]' order by city desc,region desc limit 1",1);
$x = mysqli_fetch_assoc($q);
//foreach ($x as $k => $v) echo "$k - $v<br>\n";
if ($x[country]) { $s[country] = $x[country]; $s[city] = $x[city]; $s[region] = $x[region]; $s[city_name] = stripslashes($x[city_name]); return true; }
//echo "select country,region,city,city_name from $s[pr]ip_country where start <= $s[ipnum] and end >= $s[ipnum] order by city desc,region desc limit 1";
$q = dq("select country,region,city,city_name from $s[pr]ip_country where start <= $s[ipnum] and end >= $s[ipnum] order by city desc,region desc limit 1",1);
$x = mysqli_fetch_assoc($q);
if ($x[country])
{ $s[country] = $x[country]; $s[city] = $x[city]; $s[region] = $x[region]; $s[city_name] = $x[city_name];
  $x[city_name] = addslashes($x[city_name]);
  dq("insert into $s[pr]ip_country_temp values ('$s[ipnum]','$x[country]','$x[region]','$x[city]','$x[city_name]',$s[cas])",1);
  return true;
}
else return false;
}
...
get_country();
if ($s[country])
{ $q = dq("select * from $s[pr]countries where code = '$s[country]'",1);
  $country_vars = mysqli_fetch_assoc($q);
  if ($country_vars) { $country_vars[this_flag] = "$s[site_url]/images/flags/small/$country_vars[flag]"; $country_vars[this_flag_large] = "$s[site_url]/images/flags/large/$country_vars[flag]"; }
}
return ($country_vars);
}
...
function twitter_it($item) {
global $s;
if (!$item[n]) return false;
if ((!$s[twitter_consumerKey]) OR (!$s[twitter_consumerSecret]) OR (!$s[twitter_oAuthToken]) OR (!$s[twitter_oAuthSecret])) return false;
$url = get_detail_page_url('a',$item[n],$item[rewrite_url],$item[category]);
...
function get_youtube_video_code($youtube_url) {
$youtube_url = str_replace('https','http',$youtube_url);
$youtube_url = str_replace('youtu.be/','www.youtube.com/watch?v=',$youtube_url);
...
$a = str_replace('v=','',$a);
$a = str_replace('v/','',$a);
if (trim($a)) return '<object width="560" height="315"><param name="movie" value="http://www.youtube.com/v/'.$a.'?version=3&amp;hl=en_US&amp;rel=0"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="http://www.youtube.com/v/'.$a.'?version=3&amp;hl=en_US&amp;rel=0" type="application/x-shockwave-flash" width="560" height="315" allowscriptaccess="always" allowfullscreen="true"></embed></object>';
}
...
//$style = find_style();
$t = template_select($t,$email,$s[GC_style]);
$line = parse_variables_in_template($t,$vl);
return preg_replace("/#%[a-z0-9_]*%#/i",'',$line);
}
...
while ($x=mysqli_fetch_assoc($q)) $s[$x[what]] = $x['time'];
...
{ preg_match("/(<location>)(.*)(<\/location>)/isU",$page,$x); 
  ...
  //foreach ($b as $k => $v) if (is_array($v)) echo "$k --- $v[short_name] --- $v[long_name]<br><br>\n\n"; else echo "$k --- $v<br><br>\n\n";
  //exit;
  ...
  $x = explode(' ',$in_address);
  if ((is_numeric($x[0])) AND (is_numeric($x[1]))) { $a[latitude] = $x[0]; $a[longitude] = $x[1]; }
...
{ $q = dq("select * from $s[pr]city_zip where zip = '$zip' and city like '%$city%'",1);
  if (mysqli_num_rows($q)) return mysqli_fetch_assoc($q);
}
if ($zip)
...
{ $q = dq("select rewrite_url from $s[pr]cats where n  = '$n'",1);
  $path = mysqli_fetch_row($q); $rewrite_url = $path[0];
}
...
$q = dq("select count(*) from $s[pr]ads where $where",1); $count_all = mysqli_fetch_row($q);
$q = dq("select count(*) from $s[pr]ads where $where and offer_wanted = 'offer'",1); $count_offer = mysqli_fetch_row($q);
$q = dq("select count(*) from $s[pr]ads where $where and offer_wanted = 'wanted'",1); $count_wanted = mysqli_fetch_row($q);
$q = dq("select max(created) as created,max(edited) as edited from $s[pr]ads where $where",1); $max = mysqli_fetch_assoc($q);
...
{ $usit[$x[usit_n]] = $x;
  ...
  while ($x1=mysqli_fetch_assoc($q1)) $usit_values[$x1[usit_list_n]][$x1[n]] = $x1;
}
...
{ $query = my_implode('n','or',$n);
  $q = dq("select * from $s[pr]ads_usit where n = '$n' and queue = '$queue'",1);
  while ($usit = mysqli_fetch_assoc($q))
  { for ($x=1;$x<=25;$x++)
...
while ($x=mysqli_fetch_assoc($q)) $areas[$x[n]] = $x;
return $areas;
}
...
while ($x=mysqli_fetch_assoc($q)) $categories[$x[n]] = $x;
return $categories;
}
...
while ($x = mysqli_fetch_row($q)) $a[] = $x[0];
return $a;
}
...
while ($x=mysqli_fetch_assoc($q)) $a[$x[category]] = $x;
foreach ($cats as $k=>$v)
...
while ($x=mysqli_fetch_assoc($q)) $a[$x[area]] = $x;
foreach ($areas as $k=>$v)
{ if (!$a[$v]) $a[$v][items] = $a[$v][items_offer] = $a[$v][items_wanted] = $a[$v][item_created] = $a[$v][item_edited] = 0;
  if ($offer_wanted=='offer') $a[$v][items] = $a[$v][items_offer];
  elseif ($offer_wanted=='wanted') $a[$v][items] = $a[$v][items_wanted];
...
$item = $function($n,0);
if (!$item[n]) return false;
if (!$s[no_twitter]) twitter_it($item);
if ($s[u_to_email]) dq("insert into $s[pr]u_to_email values('a','$n')",1);
...
while ($usit_list = mysqli_fetch_assoc($q)) $user_fields[$usit_list[n]] = $usit_list[usit_n];
...
dq("delete from $s[pr]orders where $query1",1);
...
while ($file = mysqli_fetch_assoc($q))
...
while ($x = mysqli_fetch_row($q)) $item[$x[0]] = $x[1];
// s novou platnosti
...
{ if (!$rewrite_url)
  { $q = dq("select rewrite_url from $s[pr]ads where n = '$n'",1);
    $x = mysqli_fetch_assoc($q); $rewrite_url = $x[rewrite_url];
  }
...
{ $q = dq("select * from $s[pr]areas",1);
  while ($area=mysqli_fetch_assoc($q)) recount_ads_cat_area($c,$area[n]);
  recount_ads_cat_area($c,0); 
}
if ($a)
{ $q = dq("select * from $s[pr]cats",1);
  while ($cat=mysqli_fetch_assoc($q)) recount_ads_cat_area($cat[n],$a);
...
{ set_time_limit(30);
  ...
  $area[path_text] = stripslashes(preg_replace("/<%.+$/i",$area[title],preg_replace("/<%.+%>/i", "",$area[path_text])));
...
$x = mysqli_insert_id($s[db]); dq("insert into $s[pr]admins_rights values('$x','admins')",1);
...
$folders = array('images','files','users','qrcodes');
foreach ($folders as $k => $v)
{ if (!is_dir("$s[phppath]/uploads/$v/$s[year]")) mkdir("$s[phppath]/uploads/$v/$s[year]");
  if (!is_dir("$s[phppath]/uploads/$v/$s[year]/$s[month]")) mkdir("$s[phppath]/uploads/$v/$s[year]/$s[month]");
}
...
{ if ($what=='u') $folder_name = 'users'; else $folder_name = 'images';
  ...
  $file_name = "$s[year]/$s[month]/$n-$file_n-$s[cas].$extension";
...
$image_width = imagesx($image);
$image_height = imagesy($image);
$put_text_x = 20;//$image_width - 300;
$put_text_x = $image_width/2 - 40;
$put_text_y = $image_height - 80;
$text = 'line 1';
imagettftext($image, $font_size, 0, $put_text_x,  $put_text_y, $font_color, $font_file, $text);
$put_text_x = $image_width/2 - 70;
$put_text_y = $image_height - 50;
$text = 'line 2';
imagettftext($image, $font_size, 0, $put_text_x,  $put_text_y, $font_color, $font_file, $text);
$put_text_x = $image_width/2 - 30;
$put_text_y = $image_height - 20;
$text = 'line 3';
imagettftext($image, $font_size, 0, $put_text_x,  $put_text_y, $font_color, $font_file, $text);
*/
########################################################################################
...
while ($x = mysqli_fetch_assoc($q)) $icons[$x[extension]] = $x[icon];
return $icons;
}
...
$q = dq("select * from $s[pr]orders where n = '$n'",1);
$order_vars = mysqli_fetch_assoc($q);
if ($order_vars[paid]) return false;
dq("update $s[pr]orders set paid = '$paid', info = '$info', notes = '$notes' where n = '$n'",1);

if ($order_vars[what]=='u')
{ $user_vars = get_user_variables($order_vars[user]);
  if ($paid)
  { $time = $order_vars[u_days] * 86400;
    if ($user_vars[paid_until]<$s[cas]) $paid_until = $s[cas] + $time; else $paid_until = $user_vars[paid_until] + $time;
    dq("update $s[pr]users set paid_until = '$paid_until' where n = '$order_vars[user]'",1);
  }
  if ($test)
  { $mysql .= "update $s[pr]ads set paid_until = '$paid_until' where n = '$order_vars[user]'\n\n";
  }
}
else
{ $ad_vars = get_ad_variables($order_vars[ad]);
  if ($paid)
  { $q = dq("select * from $s[pr]ads_orders_parts where n = '$n'",1);
    $orders_parts = mysqli_fetch_assoc($q);
    $time = $orders_parts[days] * 86400;
    foreach ($s[extra_options] as $k=>$v)
    { if (!$orders_parts[$v]) continue;
      if ($ad_vars['x_'.$v.'_by']<$s[cas]) $query .= " , x_".$v."_by = '$s[cas]' + '$time'";
      else $query .= " , x_".$v."_by = x_".$v."_by + '$time'";
    }
    if ($orders_parts[pictures])
    { if ($ad_vars[x_pictures_by]<$s[cas]) $query .= " , x_pictures_by = '$s[cas]' + '$time', x_pictures_max = '$orders_parts[pictures]'";
      else $query .= " , x_pictures_by = x_pictures_by + '$time', x_pictures_max = '$orders_parts[pictures]'";
    }
    if ($orders_parts[files])
    { if ($ad_vars[x_files_by]<$s[cas]) $query .= " , x_files_by = '$s[cas]' + '$time', x_files_max = '$orders_parts[files]'";
      $query .= " , x_files_by = x_files_by + '$time', x_files_max = '$orders_parts[files]'";
    }
    if ($ad_vars[t2]<$s[cas]) $t2 = $s[cas] + $time; else $t2 = $ad_vars[t2] + $time;
    dq("update $s[pr]ads set t2 = '$t2' $query where n = '$order_vars[ad]'",1);
  }
  if ($test) 
  { $mysql .= "update $s[pr]orders set paid = '$paid', info = '$info', notes = '$notes' where n = '$n';\n\n";
    $mysql .= "update $s[pr]ads set t2 = t2 + '$time' $query where n = '$order_vars[ad]';\n\n";
  }
}
...
{ if (is_array($v)) continue;
  $v = str_replace("''","'",$v); $form[$k] = str_replace(chr(92),'',$v); }
return $form;
}
...
<tr>
<td align="left" nowrap '.$style_queue.'>Classified ads in the Queue </td>
<td align="left"><a href="ads_list.php?action=ads_queue_home" '.$style_queue.'>'.$queue[total].'</a></td>
</tr>
<tr>
...
$a[n] = $n;
if (!$a[comments]) return '<div style="padding:10px;">'.info_line($m[no_one_comment],'<br><a href="#a_enter_comment" onclick="show_hide_div_id(0,\'comments_show_box'.$n.'\'); show_hide_div_id(1,\'enter_comment_box'.$n.'\');">'.$m[enter_comment].'</a>').'</div>';
...
<input class="form-control" style="width:'.$width.'px" name="'.$field_name.'" value="'.$field_value.'">
