...
//if (add_ajax==1) http_request.open('GET',php_script_url+'?ajax=1&'+request,true);
//else
...
function parse_ajax_request(form_id,php_script_url,element_id,add_ajax) {
var http_request = false;
//alert(form_id+'---'+php_script_url+'---'+element_id);
var request = '';
if (form_id)
...
http_request.onreadystatechange = function() { show_ajax_result(http_request,element_id); };
if (add_ajax==1) http_request.open('GET',php_script_url+'?ajax=1&'+request,true);
else http_request.open('GET',php_script_url+'?'+request,true);
http_request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
http_request.send(request);
}
