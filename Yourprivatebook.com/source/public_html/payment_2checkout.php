...
$order_vars = get_order_variables($from_2co[cart_id]);
if (!$order_vars[n]) user_home_page();
if (!$order_vars) $problem[] = $m[na_order];
else
{ if (!$from_2co['order_number']) $problem[] = $m[missing_2co_n];
  if ($from_2co['sid']!=$s[co_n]) $problem[] = $m[twoco_id_wrong];
  if ($from_2co[total]!=$order_vars[price]) $problem[] = "$m[wrong_co_price_1] $s[currency]$order_vars[price]. $m[wrong_co_price_2]$s[currency]$from_2co[total]";
...
process_order_2checkout($order_vars,$from_2co,$success,$problem);
auto_payment_done($order_vars[n]);
}

##################################################################################

function process_order_2checkout($order_vars,$from_2co,$success,$problem) {
global $s,$m;
...
$mysql = order_update_payment_info($order_vars[n],$paid,'2CheckOut',$info,$notes,$s[my_test]);
