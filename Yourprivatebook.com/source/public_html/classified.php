...
{ include('./common.php');
  ...
  { $where = get_where_fixed_part(0,0,0,0,$s[cas]);
	...
	$x = mysqli_fetch_row($q); $_GET[n] = $x[0];
  }
  show_ad_details($n);
}
...
$images = detail_page_images('a',$images[$a[n]],$a[n],0,$a);
...
if ((($a[latitude]==-1) AND ($a[longitude]==-1)) OR (($a[latitude]===0.0000000) AND ($a[longitude]===0.0000000)) OR (!trim($a[address]))) { $a[hide_map_begin] = '<!--'; $a[hide_map_end] = '-->'; }
else
{ if ( (($a[latitude]==0.0000000) OR ($a[longitude]==0.0000000)) AND (trim($a[address])) )
  { $new_ll = get_geo_data($a[address],$a[n],0);//foreach ($new_ll as $k => $v) echo "$k - $v<br>\n";
    if ( (($new_ll[latitude]==-1) AND ($new_ll[longitude]==-1)) OR ((!$new_ll[latitude]) AND (!$new_ll[longitude])) ) { $a[hide_map_begin] = '<!--'; $a[hide_map_end] = '-->'; }
  }
  $a[map_n] = $a[n];
}
...
if (check_admin_rights('ads')) $a[edit_link] = '<a target="_blank" href="'.$s[site_url].'/administration/ad_details.php?action=ad_edit&n='.$a[n].'">Edit this classified</a> (this link is available for admin only)';
...
if (!$a[owner_items]) { $a[hide_owner_items_begin] = '<!--'; $a[hide_owner_items_end] = '-->'; }
if ($s[owner_vars]) { $a[user_rateicon] = get_rateicon($s[owner_vars][rating]); $a[user_rating] = $s[owner_vars][rating]; $a[user_votes] = $s[owner_vars][votes]; }
...
{ dq("UPDATE $s[pr]ads set clicks_total = clicks_total + 1 WHERE n = '$ad_vars[n]'",0);
  ...
  if (mysqli_affected_rows($s[db])<=0) dq("insert into $s[pr]ads_stat values ('$ad_vars[n]','$ad_vars[owner]','1','1','1','1','$s[cas]')",1);
  dq("UPDATE $s[pr]ads_stat_days set i_detail = i_detail + 1 WHERE n = '$ad_vars[n]' and y = '$s[y]' and m = '$s[m]' and d = '$s[d]'",0);
  if (mysqli_affected_rows($s[db])<=0) dq("insert into $s[pr]ads_stat_days values ('$ad_vars[n]','$ad_vars[owner]','1','$s[y]','$s[m]','$s[d]')",1);
}
}
