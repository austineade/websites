...
while ($x=mysqli_fetch_assoc($q)) { $cats[$x[n]] = $x; $numbers[] = $x[n]; }
...
{ $area_vars = get_area_variables($_GET[area]);
  ...
  if (!$points)
  { $where = 'where '.get_where_fixed_part(0,$this_category,0,$_GET[area],$s[cas],$_COOKIE[GC_offer_wanted]);
    $from = get_page_from($_SESSION[this_page],$s[per_page]);
    $sortby = find_order_by_ads($_SESSION[this_sort],$_SESSION[this_direction]);
    ...
    { $pocet++;
      ...
      if ($x[picture]) $icon2 = $x[picture]; else $icon2 = "$s[site_url]/images/map_icon2.png";
      $html = '<div style="width:200px;height:150px;overflow:auto;background-color:#E0E9FE;padding:7px;color:#000000;"><img border=0 src="'.$icon2.'"> <a target="_top" href="'.get_detail_page_url('ad',$x[n],$x[rewrite_url],0).'" style="font-size:15px;font-weight:bold;">'.$x[title].'</a></div>';
      $points .= "
      var mymappoint$pocet = new google.maps.LatLng('$x[latitude]','$x[longitude]');
      createMarker(mymappoint$pocet,'$html','$x[title]','$icon1');";
    }
...
{ $ad_vars = get_ad_variables($_GET[ad]);
  if (!$ad_vars[n]) $ad_vars = get_ad_variables($_GET[ad],1);
  if (!$ad_vars[n]) exit;
  $longitude = $ad_vars[longitude]; $latitude = $ad_vars[latitude];
  $zoom = 10;
  $icon1 = "$s[site_url]/images/map_icon1.png";
  $title = $ad_vars[title];
  $ad_vars[title] = str_replace("\n", "", str_replace("\r", "", $ad_vars[title]));
  $ad_vars[description] = str_replace("\n", "", str_replace("\r", "", $ad_vars[description]));
...
<LINK href="'.$s[site_url].'/styles/Silver/bootstrap.css" rel="StyleSheet">
<style type="text/css">
html, body {
  height: 100%;
  margin: 0;
...
<script type="text/javascript"
    src="https://maps.googleapis.com/maps/api/js?key='.$s[google_map_api_key].'&sensor=false">
</script>
<script type="text/javascript">
