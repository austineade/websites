...
global $s;
$s[voted_now] = 1;
if ($_COOKIE[GC_vote][$in[n]]) { if ($s[old_poll_rate]) header ("Location: $s[back]"); else show_poll($in[n]); exit; }
$array = array(1,2,3,4,5); if ((!in_array($in[a],$array)) OR (!is_numeric($in[a])) OR (!is_numeric($in[n]))) exit;
setcookie ("GC_vote[$in[n]]",1,$s[cas]+2592000);
...
$voted_before = $_COOKIE[GC_vote][$data[n]];
for ($x=1;$x<=5;$x++)
{ if ($data["a$x"])
  { $w = $data["p$x"]+1;
    $data["vote_link_$x"] = "$s[site_url]/poll.php?action=vote&amp;n=$data[n]&amp;a=$x";
    $data["vote_argument_$x"] = "?action=vote&amp;n=$data[n]&amp;a=$x";
    if ((!$s[poll_result_after]) OR ($voted_before) OR ($s[voted_now])) $data["graph_$x"] = "<img border=0 src=\\\"$s[site_url]/images/poll_$x.jpg\\\" width=$w height=12><br />".$data["a$x"]."<br />(".$data["n$x"].")";
