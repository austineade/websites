...
{ //foreach ($_GET as $k=>$v) echo "$k - $v<br>";
  ...
  { $q = dq("select * from $s[pr]city_zip where zip = '$zip_city'",1);
    if (!mysqli_num_rows($q)) $q = dq("select * from $s[pr]city_zip where city = '$zip_city'",1);
    if (!mysqli_num_rows($q)) $q = dq("select * from $s[pr]city_zip where city like '%$zip_city%'",1);
    if ($city_vars = mysqli_fetch_assoc($q))
...
$q = dq("select count($s[pr]ads.n) $what_distance from $tables_list where $final_where $having_distance",1); $x = mysqli_fetch_row($q); $a[total] = $x[0];
  ...
  { if ($_GET[action]=='rss')
    { foreach ($item as $k=>$item1)
      { $item1[created] = date('D, j M Y H:i:s',$item1[created]+$s[time_plus]);
	    $item1[url] = get_detail_page_url('ad',$item1[n],$item1[rewrite_url],0);
        ...
        $a[individual_items] .= parse_part('rss_one_ad.txt',$item1);
      }
    }
    else
...
function search_pages_list($action,$phrases,$original_search_kind,$offer_wanted,$category,$area,$price_mark,$price,$zip_city,$radius,$usit,$total,$page,$perpage,$order_by,$sortdirection) {
global $s,$m;
...
$a[pages_list] = search_pages_list_numbers($total,$perpage,$page,"$final&order_by=$order_by&direction=$sortdirection");
...
{ if ($s[this_sort]==$v) $sort_options[] = "<button type=\"button\" class=\"btn btn-default\" disabled=\"disabled\">$m[$v]</button>";
  else $sort_options[] = '<button type="button" class="btn btn-default" onclick="location.href=\''.$base.$v.'&amp;direction='.$sortdirection.'\'">'.$m[$v].'</button>';
}
if ($total==1) { $a[hide_sortby_begin] = '<!--'; $a[hide_sortby_end] = '-->'; }

$a[sortby_options] = implode('',$sort_options);
if (!$order_by) $order_by = $s[sortby];

$a[link_asc] = "location.href='$base$order_by&amp;direction=asc'";
$a[link_desc] = "location.href='$base$order_by&amp;direction=desc'";

$a[total] = $total;

if (($total==1) OR ($total<=$perpage)) { $a[hide_pages_list_begin] = '<!--'; $a[hide_pages_list_end] = '-->'; }
...
{ $sort = str_replace(' desc','',str_replace('pick desc,','',$sort));
  ...
  for ($x=1;$x<=$pages;$x++)
  { if ($x==$page) $y .= '<button type="button" class="btn btn-default" disabled="disabled">'.$x.'</button>'; 
    elseif ((!$s[pages_max_ads]) OR (($x>=($page-$s[pages_max_ads])) AND ($x<=($page+$s[pages_max_ads])))) $y .= '<button type="button" class="btn btn-default" onclick="location.href=\''.$url.$x.'\'">'.$x.'</button>';
    if ($s[pages_max_ads])
    { if ($page>1) $link_first = '<button type="button" class="btn btn-default" onclick="location.href=\''.$url.'1\'">&laquo;&laquo;&laquo;</button>';
      if ($page<$pages) $link_last = '<button type="button" class="btn btn-default" onclick="location.href=\''.$url.$x.'\'">&raquo;&raquo;&raquo;</button>';
    }
    if ($x==($page-1))
    { $link_down = '<button type="button" class="btn btn-default" onclick="location.href=\''.$url.($page-1).'\'">&laquo;</button>';
      $s[head_pagination] .= "<link rel=\"prev\" href=\"".str_replace('&amp;','&',$url.($page-1))."\">\n";
    }
    elseif ($x==($page+1))
    { $link_up = '<button type="button" class="btn btn-default" onclick="location.href=\''.$url.($page+1).'\'">&raquo;</button>';
      $s[head_pagination] .= "<link rel=\"next\" href=\"".str_replace('&amp;','&',$url.($page+1))."\">\n";
    }
  }
  $pages_list = '<div class="btn-group btn-group-sm">'." $link_first$link_down$y$link_up$link_last ".'</div>';
}
...
{ $v = trim($v); if ((!$v) OR (strstr($v,'http://')) OR (strstr($v,'<')) OR (strlen($v)>25)) continue;
  ...
  if (mysqli_affected_rows($s[db])<=0) dq("insert into $s[pr]log_search values('$what','$v','1',NULL)",0);
}
}
...
$in[phrase0] = $in[phrase1] = $in[phrase2] = '';
...
return '<select class="form-control" name="order_by" style="width:150px;display:inline;">'.$a.'</select>';
}
...
return '<select class="form-control" name="offer_wanted">
...
return '<select class="form-control" name="direction" style="width:150px;display:inline;">
