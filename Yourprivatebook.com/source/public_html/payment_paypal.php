...
{ $v = stripslashes($v);
  if ((!preg_match("/^[_0-9a-z-]{1,30}$/i",$k)) OR (!strcasecmp($k,'cmd'))) unset ($k,$v);
...
$order_vars = get_order_variables($from_pp[item_number]);

if ($s[my_paypal_test])
{ foreach ($order_vars as $k=>$v) $email_info .= "Order data: $k - $v\n";
  mail($s[mail],'Paypal payment 5',$email_info,"From: $s[mail]");
}
if ($pp_decision=='verified')
{ if ($from_pp[mc_gross]!=$order_vars[price]) $problem[] = "$m[wrong_price_1] $order_vars[price]. $m[wrong_price_2] $from_pp[mc_gross]";
...
paypal_process_order($order_vars,$from_pp,$success,$problem,$pp_decision);
exit;
}

##################################################################################

function paypal_process_order($order_vars,$from_pp,$success,$problem,$pp_decision) {
global $s,$m;
if ($success)
...
$mysql = order_update_payment_info($order_vars[n],$paid,'PayPal',$info,$notes);
