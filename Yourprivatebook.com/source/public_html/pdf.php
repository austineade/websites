...
$table = "$s[pr]ads";
$q = dq("select * from $table where n = '$_GET[n]' and status = 'enabled'");
...
$a[user_defined] = user_defined_items_display('ad',$all_user_items_list,$all_user_items_values,$a[n],'user_item_listing.txt',0,1,0,1);
$q = dq("select * from $s[pr]files where item_n = '$_GET[n]' and what = 'a' and queue = '0' and file_type = 'image' order by file_n limit 1",1);
...
while ($x = mysqli_fetch_assoc($q)) $all_user_items_values[$x[item_n]][$x[value_code]] = $x[description];
...
{ $q = dq("select * from $s[pr]usit_list where use_for = '$use_for1' order by rank",1);
  while ($x = mysqli_fetch_assoc($q)) $all_user_items_list[] = $x;
}
