...
function ad_create_wizard($in) {
global $s,$m;
$in[c] = str_replace('_','',$in[c]);
$in[a] = str_replace('_','',$in[a]);
if ($in[c])
{ $in[c] = round($in[c]);
  $query = "parent = '$in[c]'";
}
else $query = "level = '1'";
$q = dq("select * from $s[pr]cats where submit_here = 1 and $query order by title",1);
while ($x = mysqli_fetch_assoc($q))
{ $in[categories_select] .= '<div style="float:left;width:290px;text-align:left;font-size:15px;height:30px;"><a href="'.$s[site_url].'/user.php?action=ad_create&c='.$x[n].'&a='.$in[a].'">'.$x[title].'</a></div>';
}
if ($in[categories_select]) { $in[hide_area_begin] = '<!--'; $in[hide_area_end] = '-->'; }
else
{ $category_vars = get_category_variables($in[c]);
  $in[categories_select] = "$m[selected_category]: <b>$category_vars[title]</b>";
  if (!$category_vars[submit_here]) exit;
  if ($in[a])
  { $in[a] = round($in[a]);
    $query = "parent = '$in[a]'";
  }
  else $query = "level = '1'";
  $q = dq("select * from $s[pr]areas where submit_here = 1 and $query order by title",1);
  while ($x = mysqli_fetch_assoc($q))
  { $in[areas_select] .= '<div style="float:left;width:290px;text-align:left;font-size:15px;height:30px;"><a href="'.$s[site_url].'/user.php?action=ad_create&c='.$in[c].'&a='.$x[n].'">'.$x[title].'</a></div>';
  }
  if (!$in[areas_select])
  { $area_vars = get_area_variables($in[a]);
    if (!$area_vars[submit_here]) exit;
    $s[selected_c] = $in[c];
    $s[selected_a] = $in[a];
    ad_create_form($in);
  }
...
page_from_template('ad_create_wizard.html',$in);
}
...
{ $_SESSION[GC_ad_create] = $in[c];
  ...
  elseif (($s[post_ads_who]==3) AND ($user[paid_until]<$s[cas])) problem($m[dont_right_add]);
}
elseif ($s[GC_u_n]) $user = check_logged_user();

if ($s[submit_wizard]) ad_create_wizard($in);
//if (($s[submit_wizard]) AND ((!$in[c][0]) OR (!$in[a][0]))) ad_create_wizard($in);
...
//else $in = $ad;
$in[action] = 'ad_edited';
$in[current_action] = 'ad_edit';
$category = str_replace('_','',$ad[c]); $in[c1] = get_bigboss_category($category);

$x = explode(' ',trim(str_replace('_',' ',$ad[a]))); $in[selected_a] = $x[0];
$x = explode(' ',trim(str_replace('_',' ',$ad[c]))); $in[selected_c] = $x[0];

clean_item_files('a',$in[n]);
$in[form_data] = ad_create_edit_form($in);

page_from_template('ad_edit.html',$in);
}
...
if (($s[submit_wizard]) AND (!$in[n]))
{ $category = get_category_variables(get_ad_first_category($in[c]));
  $category_vars = get_category_variables($s[selected_c]);
  $x[item_name] = $m[Category];
  $x[content] = $category_vars[title].'<INPUT type="hidden" name="c[0]" value="'.$category_vars[n].'">';
  $in[field_categories] = parse_part('form_no_field.txt',$x);
  $area_vars = get_area_variables($s[selected_a]);
  $x[item_name] = $m[Area];
  $x[content] = $area_vars[title].'<INPUT type="hidden" name="a[0]" value="'.$area_vars[n].'">';
  $in[field_areas] = parse_part('form_no_field.txt',$x);
}
else
...
{ $user_vars = get_user_variables($s[GC_u_n]);
  if (!$in[email]) $in[email] = $user_vars[email];
  if (!$in[name]) $in[name] = $user_vars[name];
  if (!$in[pub_phone1]) $in[pub_phone1] = $user_vars[phone1];
}
...
/*
  if (strstr($in[price],'USD')) $x[selected_usd] = ' selected'; else $x[selected_bob] = ' selected';
  $x[field_value] = str_replace('BOB','',$x[field_value]);
  $x[field_value] = str_replace('USD','',$x[field_value]);
  $in[field_price] = parse_part('form_field_price.txt',$x);
  <select class="form-control" name="price_currency"><option value="BOB"#%selected_bob%#>Bs</option><option value="USD"#%selected_usd%#>US$</option></select> <input maxlength="#%field_maxlength%#" size="60" name="#%field_name%#" value="#%field_value%#" class="form-control" style="width:100px;">
...
if ((!$s[GC_u_n]) AND ($s[ad_v_terms])) $in[field_terms] = parse_part('form_field_terms.txt',$x);
...
{ $b[select_boxes] .= '<select class="form-control" name="a[]">';
...
/*
<tr>
<td nowrap align="left" valign="top">Areas </td>
<td nowrap align="left" valign="top">#%select_boxes%#</td>
</tr>
*/
##################################################################################
...
if ($s[move_main_cat]) unset($bigboss);
...
{ $b[select_boxes] .= '<select class="form-control" name="c[]">';
  if ($x) $b[select_boxes] .= '<option value="0">'.$m[none].'</option>';
...
/*
<tr>
<td nowrap align="left" valign="top">Categories </td>
<td nowrap align="left" valign="top">#%select_boxes%#</td>
</tr>
*/
##################################################################################
...
{ unset($list[value]);
  ...
  { if (($_POST) AND ($in['user_item_'.$usit[n]])) $value = $in['user_item_'.$usit[n]]; //new (something entered)
    ...
    { $template = 'form_user_item_select.txt';
      ...
      $usit_value = '<select class="form-control" name="user_item_'.$usit[n].'" >'.$usit[options].'</select>';
    }   
...
$s[info] =  info_panel($m[ad_has_been_enabled]);
user_home_page();
}
...
if ($data[0])
{ $in[info] = info_panel($m[errorsfound],implode('<br>',$data[0]));
  ad_create($in);
}
...
{ if ($s[waiting]) $s[info] = info_panel($m[ad_created_waiting]);
  elseif ($s[ad_autoapr]) $s[info] = info_panel($m[new_islive]);
  else $s[info] = info_panel($m[ad_created_queue]);
  $not_include = 1;
...
if ($s[free_period]) $s[info] = info_panel($m[ad_created_user]); else $s[info] = info_panel($m[ad_created_user1]);
ad_features_edit($in);/* }*/
//else { $s[info] = info_panel($m[new_queue]); user_home_page(); }
}
...
if (!$s[ad_autoapr_user]) { $s[info] = info_panel($m[edited_queue]); user_home_page(); }
$s[info] = info_panel($m[edited_islive]);
recount_ads_for_owner($s[GC_u_n]);
user_home_page();
}
...
{ $a[to] = $s[mail]; $ad[where_hear] = $_POST[where_hear];
  $ad[page_url] = get_detail_page_url('ad',$ad[n],$ad[rewrite_url],$ad[category]);
...
if (!$in[n]) { if ((!$s[GC_u_n]) AND ($s[ad_v_terms]) AND (!$in[terms])) $problem[] = $m[read_terms]; }
...
{ if ($s[GC_u_n]) $where = "and owner = '$s[GC_u_n]'"; elseif ($in[email]) $where = "and email = '$in[email]'";
  ...
  if (mysqli_num_rows($q)) $problem[] = "You have already submitted this ad";
}
*/
...
//if ($in[price]) { if ($in[price_currency]=='USD') $in[price] = "USD$in[price]"; else $in[price] = "BOB$in[price]"; }
...
{ if ($s[GC_u_n]) $autoapprove = $s[ad_autoapr_user]; else $autoapprove = $s[ad_autoapr];
  ...
  if ($status=='enabled') $s[u_to_email] = 1;
  update_item_index('ad',$n);
  update_item_image1('a',$n);
  if ($status=='waiting')
...
$s[info] = info_panel($m[ad_confirmed]);

ad_created_edited_send_emails('created',$in[n]);
if ($s[ad_autoapr]) $s[info] = info_panel($m[new_islive]);
else $s[info] = info_panel($m[ad_created_queue]);
$not_include = 1;
include("$s[phppath]/classified.php");
...
$ad[price] = get_ad_price($in[coupon],$in[bold],$in[highlight],$in[featured],$in[home_page],$in[featured_gallery],$in[paypal],$in[xtra_pictures],$in[xtra_files],$in[xtra_videos],$in[period],$ad[created],$in[n],$ad[c],0);
if ($ad[price]<0.01) { /*$s[info] = info_panel("Order price must be higher than $s[currency]0"); */ad_features_edit($in); }
...
$ad[base_price] = $ad[price]; $ad[price] = round($ad[price] + ($ad[price]*($s[tax]/100)),2); $ad[tax] = $ad[price] - $ad[base_price];
$ad[coupon] = $in[coupon];
$ad[discount] = number_format($s[coupon_discount],2);
$ad[base_price] = number_format($ad[base_price],2);
$ad[price] = number_format($ad[price],2);
$ad[tax] = number_format($ad[tax],2);
...
$in = replace_array_text($in);
$ad = check_ad_owner($in[n]);
list ($ad[price],$payment_n) = get_ad_price($in[coupon],$in[bold],$in[highlight],$in[featured],$in[home_page],$in[featured_gallery],$in[paypal],$in[xtra_pictures],$in[xtra_files],$in[xtra_videos],$in[period],$ad[created],$in[n],$ad[c],1);
create_payment_process_record('a',$payment_n);
...
function create_payment_process_record($what,$n) {
global $s;
...
function get_ad_price($coupon,$bold,$highlight,$featured,$home_page,$featured_gallery,$paypal,$xtra_pictures,$xtra_files,$xtra_videos,$days,$created_time,$ad,$c,$insert) {
global $s;
...
if (!mysqli_num_rows($q)) $q = dq("select * from $s[pr]ads_prices where c = '0' and days = '$days'",1);
...
if ($coupon) $price = check_coupon($price,$coupon,$insert);
if ($insert)
{ $base_price = $price; $price = round($price + ($price*($s[tax]/100)),2); $tax = $price - $base_price;
  dq("insert into $s[pr]orders values(NULL,'a','$ad','$s[GC_u_n]',0,0,'$s[cas]','$price','$base_price','$tax','0','$control','','','$s[coupon_code]','$s[coupon_percent]','$s[coupon_money]')",1);
