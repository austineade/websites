...
mysql_select_db($database_AdEdgeMySql, $AdEdgeMySql);
$query_RSGetNodeList = sprintf("SELECT term_node.tid, files.filename, files.filepath, node.title, node.nid, content_type_flashnode.field_length_value, content_type_flashnode.field_long_description_value, content_type_flashnode.field_short_description_value FROM term_node LEFT JOIN node ON term_node.nid = node.nid LEFT JOIN content_type_flashnode ON term_node.nid = content_type_flashnode.nid LEFT JOIN image_attach  ON term_node.nid = image_attach.nid LEFT JOIN files ON files.nid = image_attach.iid WHERE term_node.tid = %s AND files.filename = %s AND node.status = 1", GetSQLValueString($colname_RSGetNodeList, "int"),GetSQLValueString($imgtype_RSGetNodeList, "text"));
...
<?php include('/vservers/c27464-h224072/htdocs/AdEdge/leNav_1_Colun_inc.php'); ?>
<!-- /1 Column left nav File Include -->
        </div>
        <div id="column2">
			...
			<?php if (!empty($row_RSGetNodeList)) { ?>
                <div class="box2">        
                    <div class="box2TopBlue">
                        <h1>Your search returned <?php echo $totalRows_RSGetNodeList; ?> ads with the term <?php echo $row_CategoryData['name']; ?>.</h1>
                    </div>
...
<ol>
    ...
    <li>Click <img src="/files/images/get_quote.gif" /> to get a quote for an individual ad or click <b>add to favorites</b> and build a list of ads to get quoted.</li>
                    ...
                    <div class="box2Contents">
                    <p>Sorry, your search returned no results. Please use the navigation to the right to browse other videos.</p>
					</div>					
					<?php } ?>
...
<!-- Footer File Include -->
<?php include('/vservers/c27464-h224072/htdocs/AdEdge/footer_inc.php'); ?>
<!-- /Footer File Include -->
</div>
</body>
</html>
